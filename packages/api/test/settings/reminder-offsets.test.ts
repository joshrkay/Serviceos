...
describe('normalizeReminderOffsets (Story 10.2)', () => {
  it('defaults to [24] for non-arrays / empty / all-invalid input', () => {
    expect(normalizeReminderOffsets(undefined)).toEqual([24]);
    expect(normalizeReminderOffsets(null)).toEqual([24]);
    expect(normalizeReminderOffsets('24')).toEqual([24]);
    expect(normalizeReminderOffsets([])).toEqual([24]);
    expect(normalizeReminderOffsets([0, -5, 'x', 9999])).toEqual([24]);
    expect(DEFAULT_REMINDER_OFFSETS_HOURS).toEqual([24]);
  });
