  ...
  it('technician holds ai:run and can reach POST /api/assistant/chat with a real answer', async () => {
    ...
    const app = buildApp(
      scriptedGateway([
        ...
        JSON.stringify({ content: 'Three jobs today.', autoApplied: false, proposal: null }),
      ]),
    );
    ...
    // A real answer, not a 403 and not an empty body.
    expect(res.body.message.content).toBe('Three jobs today.');
  });
