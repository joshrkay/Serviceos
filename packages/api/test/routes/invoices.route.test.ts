...
import { InMemoryInvoiceRepository, type InvoiceUpdate } from '../../src/invoices/invoice';
...
/**
 * #882 — a non-UUID `:id` used to flow straight into PgInvoiceRepository's
 * uuid comparison (`WHERE id = $1 AND tenant_id = $2`), so Postgres threw
...
class PgLikeInvoiceRepository extends InMemoryInvoiceRepository {
  ...
  async update(tenantId: string, id: string, updates: InvoiceUpdate) {
    if (!UUID_RE.test(id)) {
      throw new Error(`invalid input syntax for type uuid: "${id}"`);
    }
...
function buildPgLikeInvoicesApp(): Express {
  const app = express();
  app.use(express.json());
  app.use((req: Request, _res: Response, next: NextFunction) => {
    (req as AuthenticatedRequest).auth = {
      userId: TEST_USER_ID,
      sessionId: 'session-invoices-882',
      tenantId: TEST_TENANT_ID,
      role: 'owner',
    };
    next();
...
describe('malformed :id never reaches Postgres as a raw uuid comparison (#882)', () => {
  ...
  beforeEach(() => {
    app = buildPgLikeInvoicesApp();
  });

  it('GET /api/invoices/not-a-uuid returns 404 NOT_FOUND, not 500', async () => {
    const res = await request(app).get('/api/invoices/not-a-uuid');
    expect(res.status).toBe(404);
    expect(res.body.error).toBe('NOT_FOUND');
    expect(res.body.message).toBe('Invoice not found');
  });

  it('PUT /api/invoices/not-a-uuid returns 404 NOT_FOUND, not 500', async () => {
    const res = await request(app).put('/api/invoices/not-a-uuid').send({});
    expect(res.status).toBe(404);
    expect(res.body.error).toBe('NOT_FOUND');
  });

  it('PATCH /api/invoices/not-a-uuid returns 404 NOT_FOUND, not 500', async () => {
    const res = await request(app).patch('/api/invoices/not-a-uuid').send({});
    expect(res.status).toBe(404);
    expect(res.body.error).toBe('NOT_FOUND');
  });

  it('POST /api/invoices/not-a-uuid/payment-link returns 404 NOT_FOUND', async () => {
    const res = await request(app).post('/api/invoices/not-a-uuid/payment-link').send({});
    expect(res.status).toBe(404);
    expect(res.body.error).toBe('NOT_FOUND');
  });

  it('POST /api/invoices/not-a-uuid/issue returns 404 NOT_FOUND, not 500', async () => {
    const res = await request(app).post('/api/invoices/not-a-uuid/issue').send({});
    expect(res.status).toBe(404);
    expect(res.body.error).toBe('NOT_FOUND');
  });

  it('POST /api/invoices/not-a-uuid/payment returns 404 NOT_FOUND', async () => {
    const res = await request(app)
      .post('/api/invoices/not-a-uuid/payment')
      .send({ amountCents: 100 });
    expect(res.status).toBe(404);
    expect(res.body.error).toBe('NOT_FOUND');
  });

  it('POST /api/invoices/not-a-uuid/transition returns 404 NOT_FOUND, not 500', async () => {
    const res = await request(app)
      .post('/api/invoices/not-a-uuid/transition')
      .send({ status: 'issued' });
    expect(res.status).toBe(404);
    expect(res.body.error).toBe('NOT_FOUND');
  });

  it('POST /api/invoices/not-a-uuid/send returns 404 NOT_FOUND', async () => {
    const res = await request(app).post('/api/invoices/not-a-uuid/send').send({});
    expect(res.status).toBe(404);
    expect(res.body.error).toBe('NOT_FOUND');
  });

  it('a well-formed but unknown uuid still answers the ordinary 404', async () => {
    const res = await request(app).get('/api/invoices/11111111-1111-1111-1111-111111111111');
    expect(res.status).toBe(404);
    expect(res.body.error).toBe('NOT_FOUND');
  });
});
