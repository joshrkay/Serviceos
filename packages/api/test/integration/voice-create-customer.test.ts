...
describe('Integration — voice create_customer via ProposalExecutor', () => {
  ...
  it('persists customer on execute and returns it via GET /api/customers/:id', async () => {
    ...
    // handler emits a second proposal.executed join-row (AC-4) tying the
    // executed proposal to the created customer.
    ...
    const executedEvents = audits.filter((e) => e.eventType === 'proposal.executed');
    expect(executedEvents).toHaveLength(1);
    expect(executedEvents[0].entityType).toBe('customer');
