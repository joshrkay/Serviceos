 ...
 *
 * Both counters must be 0. This is a pure adapter-behavior check against
 * hand-rolled fakes — no Postgres. It moved here (unit lane, `npm test`) from
 * `test/integration/` (#1014, G3): a file under `test/integration/` that
 * never opens a pool is not proof, however many paths it covers. The
 * complementary real-Postgres proof — the RV-130 `consent_events` ledger row
 * is written only once the disclosure has actually played, never before, and
 * never leaks across tenants — lives in
