...
export interface EstimateEditDelta {
  id: string;
  tenantId: string;
  estimateId: string;
...
export interface EditDeltaRepository {
  create(delta: EstimateEditDelta): Promise<EstimateEditDelta>;
  findByEstimate(tenantId: string, estimateId: string): Promise<EstimateEditDelta[]>;
}
...
export function computeEstimateDeltas(
  oldSnapshot: EstimateSnapshot,
  newSnapshot: EstimateSnapshot
): DeltaEntry[] {
...
export function summarizeDeltas(deltas: DeltaEntry[]): string {
...
export async function createEditDelta(
  tenantId: string,
  estimateId: string,
  fromRevisionId: string,
  toRevisionId: string,
  oldSnapshot: EstimateSnapshot,
  newSnapshot: EstimateSnapshot,
  repository: EditDeltaRepository
): Promise<EstimateEditDelta> {
  const deltas = computeEstimateDeltas(oldSnapshot, newSnapshot);

  const editDelta: EstimateEditDelta = {
    id: uuidv4(),
    tenantId,
    estimateId,
    fromRevisionId,
...
export class InMemoryEditDeltaRepository implements EditDeltaRepository {
  private deltas: EstimateEditDelta[] = [];
  ...
  async findByEstimate(tenantId: string, estimateId: string): Promise<EstimateEditDelta[]> {
    return this.deltas
      .filter((d) => d.tenantId === tenantId && d.estimateId === estimateId)
