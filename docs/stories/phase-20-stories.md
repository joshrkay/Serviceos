...
**What shipped:** Two RLS tables — `invoice_dunning_configs` (one per tenant: ordered `reminder_steps` JSONB; `late_fee_type none|flat|percent`, value in cents/bps, grace days, optional cap) and `invoice_dunning_events` (`(tenant, invoice, kind, step_index) UNIQUE` idempotency ledger). Repos mirror the `service_agreement_runs` pattern (in-memory raises `23505` on duplicate). `defaultDunningConfig()` gives a conservative single-3-day-SMS fallback.
...
**Build prompt:** For each overdue invoice, load the tenant config (fallback `defaultDunningConfig`), read sent step indexes via `dunningEventRepo.findByInvoice`, call `selectDueReminderSteps`, send each step's channel via `transactionalComms`, then insert the `invoice_dunning_events(kind='reminder', step_index, channel)` dedup row (catch `23505` as "already sent"). Keep the existing single `invoice.overdue` audit on first crossing.
...
**Build prompt:** When `computeLateFeeCents > 0` and no `late_fee` dunning event exists for the current period, append a late-fee line item (category `other`, `taxable=false`) via `billing-engine`, recompute totals, refresh money-state, and insert `invoice_dunning_events(kind='late_fee', step_index, amount_cents)`. Default = surface as an `update_invoice` proposal (money-affecting) — auto-apply only if the tenant opts in. Cap via `lateFeeMaxCents` using the sum of prior `late_fee` event `amount_cents`.
