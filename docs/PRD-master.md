...
**Date:** 2026-07-12
**Owner:** Product
**Status:** Canonical positioning + interaction spec. Companion to `docs/PRD.md` (v2.0, the canonical product reference); supersedes the separate competitive, voice, decision, and story drafts from this cycle. *(Drafted alongside PRD v3.0, which is archived at `docs/archive/2026-07-cleanup/docs/PRD-v3.md`.)*
...
**25 voice-reachable intents** have both a speech→proposal task handler and an approval→persistence execution handler wired in production. Grouped by **action class**, which decides what may *ever* execute without a tap.

> **Live source of truth:** `docs/reference/voice-action-catalog.md` (test-pinned to the code). At landing time it lists **34** speakable intents — the Appendix A on-ramps (crew add/remove, batch-invoice, collections) have shipped since this document's 2026-06-14 audit basis. The catalog below is the positioning snapshot; the reference doc wins on current state.
...
A contract test imports `SUPPORTED_INTENTS`, `INTENT_TO_PROPOSAL_TYPE`, the execution-registry keys, and `actionClassForProposalType`, and **fails when the published catalog drifts from the code.**
...
| **Price, 2-person shop** | $79–249/mo | Prohibitive | Add-on cost | **See §14 — unresolved** |
...
> **Build-status discipline is non-negotiable.** The docs lag the code. Every gap below was code-verified against the 2026-06-14 audit; before acting on (or quoting) a gap, re-verify against `docs/reference/voice-action-catalog.md` and `/packages` — Gap 3's on-ramps have already shipped, and Gap 2 has partially closed (`draft_invoice` now has integration persistence proof).
...
### 🟠 Gap 2 — *"It looked like it worked but nothing saved."*
Several execution handlers **degrade to a synthetic-id passthrough** when a dependency is unwired: they return `{ success: true }` and **persist nothing.**

Only `create_appointment` and `create_customer` have integration proof that a spoken action actually lands a row + audit event in real Postgres. **`draft_invoice` and `create_job` — the two most valuable spoken actions — are mocked-only.** Audit emission on the create paths is best-effort and asserted by **no test**; a refactor that drops it passes green.

**Fix:** Docker-gated integration tests for invoice/job/appointment persistence + audit, and a **boot-time wiring guard** that fails startup loudly if any voice-reachable handler is degraded while a Postgres pool is configured. **Certify before building more.**

### 🟠 Gap 3 — Sentences the owner would obviously say that get silently skipped.
These have proposal types, Zod schemas, and execution handlers **already built** — but no classifier intent and no `INTENT_TO_PROPOSAL_TYPE` entry, so **the transcript is dropped on the floor**:
...
**Front-half-only builds.** No migrations, no new handlers. **Cheapest voice wins on the board — see Appendix A.**
...
The **certified** path that actually books an appointment is **Twilio Gather**. **VAPI does not book appointments yet.** Media Streams is the real-time path that hits the sub-800ms bar. **Sales and engineering must not describe these as one thing.** → §14.
...
> **Numbering note:** these open decisions are registered in `docs/decisions.md` as **D-020, D-021, D-022** (the log's D-013–D-019 were already taken by earlier decisions). Earlier drafts of this document numbered them D-013/D-014/D-015.
...
### The contradiction
The code hard-refuses recorder-channel voice approval (RV-071/225; launch scope = tap). The PRD (v3.0, archived at `docs/archive/2026-07-cleanup/docs/PRD-v3.md`) promises it in four places:
...
This is systemic, not a typo. **It is our most quotable line, and it does not work.**
...
- **Docs:** the four claims above live in the archived `PRD-v3.md`, which is frozen — no archive edits. The Option-A amendments apply to the *active* narrative surfaces: this document (already consistent), `docs/PRD.md`, and GTM copy. The Jenna narrative becomes *"The proposals are waiting when she stops. She approves them with a thumb, and dictates any edits."* `docs/launch/voice-interaction-scope.md` **unchanged — it was right.**
...
The archived `PRD-v3.md` says **$300–500/mo** (full back office). A **$99/mo** figure also exists in the record. **Unresolved.** Competitive anchor: Jobber $79–249/mo for a 2-person shop; Avoca is an add-on *on top of* whatever runs the office.
...
CERTIFY FIRST ─────────────────────────────────────────────
  ...
  U4  Audit-emission assertions on all three create paths
  U5  Boot-time execution-wiring guard  ⚠ blocks U8

THEN BUILD ────────────────────────────────────────────────
  U6  Crew add/remove          capture     ← ship first
  U7  Batch-invoice            capture
  U8  Collections (late fee, reminder, nudge)   money/comms   ⚠ held for review
...
**Recommended dispatch now:** **U6 + U7** (clean capture-class wins, prove the pattern), **U5 in parallel**, then **U8** once the double-send decision is made and the money-class commit is isolated for review.

---

# Appendix A — Voice on-ramp stories (U6–U8)

**Parent plan:** `docs/archive/2026-07-cleanup/docs/plans/2026-06-14-001-feat-voice-action-pipeline-audit-and-gap-buildout-plan.md` *(archived)* · Requirement **R5**
...
**Invariants:** integer cents (reuse `packages/api/src/shared/billing-engine.ts`) · `tenant_id` + RLS unchanged · audit event on every mutation · classification via the **LLM gateway** only · proposals stay typed, human-approved, **never auto-executed**.
...
- **Persistence certification** — U2/U3/U4. `draft_invoice` and `create_job` are still **mocked-only.** That work is **separate from, and more urgent than, these on-ramps.**
...
A later competitive audit found **44 of 48 claims fully built**, with three concrete shipping gaps — **conversational onboarding loop, delayed post-job thank-you SMS, painting vertical pack** — and one documentation-only mismatch. See also `docs/competitive-review-rivet-vs-jobber-2026-07-02.md` for the most recent code-verified pass, including findings on Jobber's 2026 AI capabilities that must be reconciled with §12 before this scorecard goes in front of a prospect.

## Sources

`docs/archive/2026-07-cleanup/docs/PRD-v3.md` *(archived)* — voice-first architecture, locked decisions, feature map, personas, parity map, competitive summary · `docs/strategy/day-in-the-life.md` (personas, bad-day failure modes) · `docs/competitive-gap-analysis.md` · the **voice-action-pipeline audit plan** 2026-06-14 *(archived; see Appendix A)* — the 25-intent capability matrix, RV-071/225 approval refusal, persistence gaps, unspeakable actions · the **voice-first-onboarding plan** (dormant orchestrator, route-aware VoiceBar) · the **PRD-gap-closure plans** (multi-turn onboarding gap, gateway multimodal) · the **appointment-verification plan** (Gather vs. VAPI) · the **launch-quality-bar plan** (voice smoke test, load SLOs) · the **CRM/comms/multi-location parity plan** · `docs/launch/voice-interaction-scope.md` (PTT at launch; voice approval post-launch; one-tap SMS approval locked) · `docs/decisions.md` (D-003 cents, D-004 no auto-execute, D-005 gateway, D-013 build-status correction).
