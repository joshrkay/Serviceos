...
**Residual (follow-up):** extend the fence to the lower-risk `context-builder.recentMessages` path (RAG query + prompt), and add an explicit `untrusted` provenance column at the transcript/message ingest layer — today untrusted-ness is derived from `senderRole='customer'` / `speaker='caller'` at read time, which travels with the row but is not the dedicated lifetime flag the contract's strict reading asks for.
