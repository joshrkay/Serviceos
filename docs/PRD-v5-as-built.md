...
| **1.3** | **As T**, I want a price book I didn't have to build, so I can quote on day one | **Given** I pick HVAC, **when** the pack activates twice concurrently, **then** one seeding occurs under an advisory lock and my catalog holds the pack's SKUs at the pack's prices | **5** ↑ (T2·T3) — a real browser pack pick, two concurrent activations seed once under the advisory lock, catalogs disjoint per tenant | **D:** `onboarding-pack.test.ts`, `onboarding-pack-seed-concurrency.test.ts` — *no audit assertion* · **G1 2026-09-12:** 3/3 + 1/1 ✓ (the #976 flake did not reproduce); `PgAuditRepository` is wired but no audit event is asserted (4− class); **T0** → 3 · **#1016 2026-09-12:** `InMemoryAuditRepository` swapped for `PgAuditRepository` in `onboarding-pack.test.ts` + `onboarding-pack-seed-concurrency.test.ts`; `tenant.pack_activated` row read back; T1 (a second tenant neither sees nor is seen); T3 (two tenants, two DIFFERENT packs, two price books in one run — 6 HVAC vs 6 plumbing catalog rows, zero overlap). `EXTERNAL_TEST_DB_URL=<kept container> RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts test/integration/onboarding-pack.test.ts test/integration/onboarding-ai-check.test.ts test/integration/onboarding-conversation-concurrent-turns.test.ts test/integration/onboarding-conversation.test.ts test/integration/onboarding-pack-seed-concurrency.test.ts test/integration/onboarding-status-derived-gate.test.ts test/integration/onboarding-vapi.test.ts` → 7 files, 29/29 ✓ at real Postgres, audit rows read back through `PgAuditRepository`. PROVEN-REAL-DB → 4; rung 5 = the hermetic onboarding journey (separate lane) · **§8.1 rung-5 lane (Fable gate on PR #1137, 2026-09-13 18:42Z):** `onboarding-pack-concurrency.spec.ts` → 1 passed: owner picks HVAC in the real browser; a second tenant fires two concurrent `POST /api/onboarding/pack` and seeds exactly once (`1.3-race-pack-activations`); T3 disjoint catalogs (`1.3-T3-catalog-disjoint`); T2 bystander untouched. → **5 (T2·T3)** |
| **1.4** | **As T**, I want to close the laptop mid-setup and come back to exactly where I was, so onboarding survives a service call | **Given** a partially-configured tenant, **when** I return, **then** the current step is derived from *facts about my tenant* — never a stored wizard flag — and cannot disagree with reality | **5** ↑ (T2) — the current step is derived from tenant facts (pack before identity) and survives a reload | **D:** `onboarding-status.test.ts` ✅ *executed* · **G1 2026-09-12:** 3/3 ✓; no audit event asserted (4− class); **T0** → 3 · **second-tenant lane (Fable gate) 2026-09-12:** `onboarding-status.test.ts` — a neighbour tenant driven to full completion (settings + pack + AI check + Twilio integration + subscription + a voice session) leaves tenant A's `GET /status` at `currentStep: 'identity'`, `isComplete: false`. `RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts test/integration/onboarding-status.test.ts` → 4/4 ✓ (RED: expected null). The derivation is a read over real rows; there is no write to audit, so the 4− class does not apply — T1 → **4** (Fable's reading) (PR #1074) · **§8.1 rung-5 lane (Fable gate on PR #1137, 2026-09-13 18:42Z):** `onboarding-status-gate.spec.ts` (1.4 case) → the out-of-order pack-before-identity tenant derives the right step and keeps it after reload (`1.4-after-reload-derived-step.png`, `1.4-out-of-order-status`); neighbour independent. → **5 (T2)** |
| **1.5** | **As T**, I want to look around before finishing setup, so I can decide if this is worth my time | **Given** identity is saved, **when** I navigate anywhere, **then** the CRM is unlocked and later steps nudge rather than hard-block | **5** ↑ (T2) — pre-identity /customers redirects, post-identity the CRM is unlocked, /inbox exempt; neighbour independent | soft-gate logic; no DB-level proof · **G1 2026-09-12:** **NO-COMMAND** — no test targets the soft gate; 10 adjacent onboarding-gate unit files pass 67/67. Rung provisional · **#1016 2026-09-12:** new `test/integration/onboarding-status-derived-gate.test.ts` — a brand-new tenant can look around (GET /status 200 with the real incomplete state, never a "finish setup first" block); completing pack BEFORE identity still derives correctly (status reads facts, not a stored wizard pointer); `PUT /identity` lands `tenant.identity_set` through `PgAuditRepository` and the next /status reflects it; T1 (a second tenant looking around sees none of the first's partial setup or audit trail). `EXTERNAL_TEST_DB_URL=<kept container> RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts test/integration/onboarding-pack.test.ts test/integration/onboarding-ai-check.test.ts test/integration/onboarding-conversation-concurrent-turns.test.ts test/integration/onboarding-conversation.test.ts test/integration/onboarding-pack-seed-concurrency.test.ts test/integration/onboarding-status-derived-gate.test.ts test/integration/onboarding-vapi.test.ts` → 7 files, 29/29 ✓ at real Postgres, audit rows read back through `PgAuditRepository` → 4 · **§8.1 rung-5 lane (Fable gate on PR #1137, 2026-09-13 18:42Z):** `onboarding-status-gate.spec.ts` (1.5 case) → the real `OnboardingGuard` soft gate: `/customers` hard-redirects before identity and unlocks after (`1.5-pre-identity-customers-redirect.png`, `1.5-post-identity-customers-unlocked.png`), T2 (`1.5-T2-tenant-settings`). → **5 (T2)** |
...
| **1.7** | **As T**, I want a 14-day trial with no card surprises, so I can try this without commitment | **Given** a plan whose price fails live Stripe validation, **when** checkout renders, **then** that plan is **omitted, never shown at a wrong price** | **4** ↑ (T1) — reached: unconfigured plans fail closed (503, never a broken plan card); 5 needs the live-Stripe price-validation leg (#1000/#1002 parking lot) | **D:** subscription integration tests · **G1 2026-09-12:** the proofs are `integration/billing-trial.test.ts` (6/6 ✓, T0) and `integration/trial-provisioning-first-value.test.ts` (6/6 ✓, **T1** — "cross-tenant isolated", `otherTenant` at :658) · **§8.1 rung-5 lane (Fable gate on PR #1137, 2026-09-13 18:42Z):** `onboarding-billing-plan-validation.spec.ts` → 2 passed (one an expected-fail pin): the real `/api/onboarding/billing/plans` route fails closed and the checkout renders no broken card (`1.7-billing-plans-omitted.png`); the acceptance's "price fails LIVE Stripe validation" branch (`billing/subscription.ts:293-345`) needs a live key → pinned, parked. → **4 (T1)** reached |
| **1.8** | **As T**, I want proof the AI works before I point my real number at it, so I'm not experimenting on customers | **Given** verification runs, **when** it passes, **then** the step completes with DB status `passed`; **when** it fails, **then** I get an `ai_verification_failed` blocker and a retry that resets to pending and re-enqueues | **5** ↑ (T2·T3) — the whole onboarding journey reached hermetically to a passing AI check, a neighbour on a different pack completing its own check concurrently changes nothing | **D:** `onboarding-ai-check.test.ts` ✅ *executed* · **G1 2026-09-12:** 3/3 ✓ but the audit leg is `InMemoryAuditRepository` (×3) — 4− class — and **T0** → 3 · **#1016 2026-09-12:** `onboarding-ai-check.test.ts` now reads `tenant.ai_verified` / `ai_verification_failed` / `ai_verification_retry` back through `PgAuditRepository` with a T1 negative (tenant A's entity id under tenant B's scope → empty). PROVEN-REAL-DB (the AI-check write + its audit row at real Postgres) with T1 → 4 · **§8.1 lane (Fable gate) 2026-09-13:** `e2e/journeys/onboarding-identity.spec.ts` (1.8 test) — a fresh owner bootstrapped through the real Clerk webhook walks the real `/onboarding` UI: identity → pack → phone (the dev Twilio stub provisioning runs for real on the in-process queue) → billing (the UI's own *Start 14-day free trial* button is disabled hermetically — "No billing plans are currently configured" — so the trial is entered through the real Stripe webhook route the product listens on, a self-signed trial event that mirrors the subscription to `trialing`, which is what enqueues `verify_ai`) → the AI check passes at real Postgres with its `tenant.ai_verified` audit row, the hermetic `MockLLMProvider` standing in for the model exactly as `app.ts` does without a key; a neighbour tenant is untouched (T1). `CLERK_DEV_HMAC_TOKENS=true DB_SSL=false DATABASE_URL=<testcontainer> E2E_USE_TEST_DB=true VITE_CLERK_PUBLISHABLE_KEY=… TWILIO_* TENANT_ENCRYPTION_KEY=… STRIPE_SECRET_KEY=<any> STRIPE_WEBHOOK_SECRET=… npx playwright test e2e/journeys/onboarding-identity.spec.ts --project=chromium --retries=0` → 2 passed (45.1s) (real Postgres, chromium project; the test skips itself without both Stripe variables — two gate runs learned that). Screenshots `setup-8-1/1.8-billing-reached.png`, `1.8-ai-verified-past.png`. Reached + T1 → **4** stays; rung 5 needs the T2 leg (a neighbour's AI-check state must not change this tenant's answer) — one more assertion in the same spec. 1.10 investigated by the same lane: the Brand-voice Settings row renders only behind `brand_voice_configurator_enabled` with no self-service enable path → blocked-on-josh (4 is its ceiling until decided); 1.6 stays 3 pending a live Twilio subaccount (blocked-on-josh) · **T2 leg (Fable gate) 2026-09-13 04:24Z:** the same spec's 1.8 test now drives a neighbour tenant through its OWN identity → pack (plumbing, vs. this tenant's HVAC) → phone → billing (its own signed trial webhook) → AI check over the API, seeded before and completed during this tenant's browser journey; at real Postgres the neighbour ends with exactly its own `tenant.ai_verified` row (not 0, not 2), its own `ai_check: done` and a disjoint catalog, while this tenant's audit count stays exactly 1 — `npx playwright test e2e/journeys/onboarding-identity.spec.ts --project=chromium --retries=0` at head `b2ed4cc01` → 2 passed (45.9s); snapshot `setup-8-1/1.8-T2-neighbour-audit-events.snapshot.txt`. Reached hermetically + T2 (divergent neighbour data) + T3 (per-tenant pack/catalog read stays disjoint) → **5**. Still recorded, not a shortcut: the trial is entered through the app's own signed Stripe webhook route because the UI button is disabled hermetically ("No billing plans are currently configured") |
| **1.9** | **As T**, I want to set this up by talking instead of filling a form, because I'm doing it in the truck | **Given** the conversational path, **when** I complete up to 15 turns, **then** transcript, extractions and clarification counts round-trip through JSONB, RLS is ENABLE + FORCE, and cross-tenant reads are refused — with the form wizard still available as fallback and edit surface | **5** ↑ (T2) — up to 15 real turns in the "Talk it through" panel; transcript/extractions/counts round-trip JSONB, RLS ENABLE+FORCE, cross-tenant refused both ways, form fallback per tenant; genuine LLM extraction pinned (#1119-class) | **D:** `onboarding-conversation.test.ts` ✅ *executed* — *no audit assertion* · **G1 2026-09-12:** 6/6 ✓, **T1** (`secondTenant` :35/:54); no audit event asserted at all — 4− holds; the audit leg, not a neighbour, stands between it and 4 · **#1016 2026-09-12:** conversation turns audited at real Postgres (`agent.onboarding.advanced` / `extractor_called` / `review_confirmed` rows read back through `PgAuditRepository`), invisible under a second tenant (T1); the Vapi activation leg likewise (`onboarding-vapi.test.ts`). `EXTERNAL_TEST_DB_URL=<kept container> RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts test/integration/onboarding-pack.test.ts test/integration/onboarding-ai-check.test.ts test/integration/onboarding-conversation-concurrent-turns.test.ts test/integration/onboarding-conversation.test.ts test/integration/onboarding-pack-seed-concurrency.test.ts test/integration/onboarding-status-derived-gate.test.ts test/integration/onboarding-vapi.test.ts` → 7 files, 29/29 ✓ at real Postgres, audit rows read back through `PgAuditRepository` → 4 · **§8.1 rung-5 lane (Fable gate on PR #1137, 2026-09-13 18:42Z):** `onboarding-conversation-hermetic.spec.ts` → 2 passed (one an expected-fail pin): 15 real turns to `capped` via the FSM caps, JSONB round-trip, RLS ENABLE+FORCE confirmed, cross-tenant session reads refused 404 both directions, form fallback reachable; T2 — the neighbour runs its OWN divergent conversation and each tenant's session holds only its own turns (`1.9-session-terminal-state`, `1.9-T1-neighbour-no-session` + the T2 leg at head d9ca0b8af). The model's extraction quality is the hermetic mock's (`ai/providers/mock.ts:223-232`) — pinned, #1119-class. → **5 (T2)** |
| **1.10** | **As M**, I want the AI to sound like my shop and then **stop changing**, so my customers hear one voice | **Given** six captured fields, **when** the first write lands, **then** the voice locks; every later edit is cool-down gated under a `FOR UPDATE` lock with no lost update, rollback never mutates history, and a spoken *"lock my brand voice"* can **never** set the lock | **4** ↑ — re-confirmed ceiling: the brand-voice Settings row is behind a tenant flag with no self-service path (decision parked) | **D:** `brand-voice.integration.test.ts`, `update-brand-voice-voice-execution.test.ts` ✅ *executed* · **G1 2026-09-12:** 5/5 + 6/6 ✓, **T1** (`brand-voice.integration.test.ts:264` "does not expose the brand voice to another tenant") · **§8.1 rung-5 lane (Fable gate on PR #1137, 2026-09-13 18:42Z):** `onboarding-brand-voice-gate.spec.ts` → 1 passed: flag off for every tenant on main, Settings never renders the row (`1.10-settings-no-brand-voice-row.png`), the real capability-toggle route refuses `brand_voice_configurator`. Nothing an owner can reach → stays **4** |
...
| # | User story | Acceptance criterion | Rung | Confirm |
|---|---|---|---|---|
| **2.1** | **As M**, I want my phone answered 24/7 in my shop's voice, so I stop losing jobs to whoever answers first | **Given** a customer dials my number, **when** the call lands, **then** the tenant is resolved from the real `phoneE164` column and the greeting is my shop's | **5** ↑ (T3) — each dialled number resolves its own tenant and greets with that tenant's own business name; two tenants with different names each hear their own greeting in the same run | **D:** `voice-inbound-appointment.test.ts -t "routes a dialed number"` · **G1 2026-09-12:** `voice-inbound-appointment.test.ts` 3/3 ✓ with audit events (the `-t` title is a substring of "routes a dialed number to its tenant via the real phoneE164 column"); its only neighbour mentions are comments — **T0** — and no reachability run backs 5 → 3 · **#1014-A 2026-09-12:** `voice-inbound-appointment.test.ts` gains T2 — tenant B runs its own complete inbound-call flow (own DID, customer, location, job, booking) through the same production path without touching tenant A's rows or availability; audit rows (`appointment.created`, `proposal.executed`) read back. `EXTERNAL_TEST_DB_URL=<kept container> RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts <the 9 touched integration files>` → 9 files, 32/32 ✓ at real Postgres → 4 (T2) · **§8.2 rung-5 lane (Fable gate on PR #1148 @ 1bb403231, 2026-09-13 20:23Z; merged fc267cdb4):** `e2e/telephony-capture-2-1.spec.ts` (signed `/api/telephony/voice`, dedicated api port, real Postgres) → 2 passed: tenant A's DID greets with A's own name, tenant B's DID with B's, same run (T2). A tenant that never wrote `tenant_settings.voice_greeting` hears `TWILIO_BUSINESS_NAME ?? 'our team'` (`app.ts:3304/3611`) → "the greeting is my shop's" is not met by default → **4 (T2)** until #1156 · **Re-grade at main (Fable, origin/main @ 792f8d2b9, 2026-09-14 18:02Z):** `e2e/telephony-capture-2-1.spec.ts` (signed `/api/telephony/voice`, dedicated api port, real Postgres, tenants bootstrapped through the API) → 2 passed: tenant A's dialled number greets with A's own business name, tenant B's with B's, same run; `test/integration/greeting-tenant-business-name-1156.test.ts` → 2/2 ✓ (neither tenant hears the env fallback). #1156 fixed by #1167 → **5 (T3)** · **Row artifact at main (Fable, origin/main @ 859db9f94, 2026-09-15 19:19Z):** `e2e/telephony-capture-2-1.spec.ts` (phone env, api :38580; a BEFORE TRUNCATE trigger kept the Playwright teardown from emptying the test DB before the dump) → 2 passed (13.2s); rows: `tenant_settings` d9f6916b "Alpha Rooter and Drain 21" / 79a0263a "Bravo HVAC Services 21"; `tenant_integrations` twilio `+15121107201` → d9f6916b, `+15121107202` → 79a0263a; `voice_sessions` 1 `voice_inbound` per tenant; audit per tenant 1 each of `agent.calling.idle.incoming_call`, `agent.calling.greeting.greeted_ok`, `agent.calling.identifying.unknown_caller`, `lead.created`, `recording_consent.granted`. The spoken business name is in the TwiML the spec asserts, not in a row. Evidence `regrade-evidence3/telephony-capture-2-1-1-rows.txt`; grade cell unchanged |
| **2.2** | **As M**, I want callers told they're being recorded **before** anything is captured, so I'm not exposed in a two-party-consent state | **Given** an inbound call, **when** the greeting plays, **then** the disclosure precedes `<Start><Record>`, Media Streams consumes no audio until it has played, **and the implicit-consent ledger row is written** | **4** ↑ (T2) — reached on the Gather transport: the disclosure precedes `<Start><Record>`, the consent ledger row and `recording_consent.granted` land; the Media Streams ordering clause stays unit-level (transport off by default) | Ordering proven; **the ledger write is a `vi.fn()`** in a file that never opens a pool. **S:** `grep -c "new Pool(" …/conversation-consent-ordering.test.ts` → **0** · **G1 2026-09-12:** 13/13 ✓ — but only under the integration config (`vitest.config.ts` excludes `test/integration/**`, and the file opens no pool): PROVEN-UNIT, 3 stands · **#1014-A 2026-09-12:** `conversation-consent-ordering.test.ts` now opens the real pool (its 13 adapter-ordering assertions moved verbatim to `test/telephony/media-streams/`, so the NO-DB baseline SHRANK by one — G3): the `consent_events` row is written at the disclosure-PLAYED point, strictly before any caller audio is captured; a fail-closed disclosure writes no row; T1 (tenant B's consent never satisfies tenant A's gate for the same number). No `audit_events` emission exists on this path (`consent_events` is the append-only ledger per its docstring) → REAL-DB-WRITE-ONLY 4− (issue filed); phone-surface reachability is lane C. `EXTERNAL_TEST_DB_URL=<kept container> RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts <the 9 touched integration files>` → 9 files, 32/32 ✓ at real Postgres · **Audit leg landed (Fable gate on PR #1136, 2026-09-13 16:15Z):** `commitRecordingConsent` (`telephony/twilio-adapter.ts`) emits `recording_consent.granted` through the adapter's wired audit repo (system actor, call/tenant in metadata); `conversation-consent-ordering.test.ts` asserts it at real Postgres, T1 (tenant B's consent never satisfies A's gate). REAL-DB-WRITE + audit → **4 (T1)**. RED (main's six product files restored, the branch's five integration files): 12 failed / 11 passed; GREEN: 23/23 at real Postgres, unit 2130/2130, tsc 0 errors introduced; the new event rows read back per tenant from the GREEN container. Emitters are best-effort (logged, never masking the real outcome) like their sibling emitters on the same mid-call / sweep / hook paths. · **§8.2 rung-5 lane (Fable gate on PR #1148 @ 1bb403231, 2026-09-13 20:23Z; merged fc267cdb4):** `telephony-capture-2-2.spec.ts` → 2 passed: disclosure before `<Start><Record>` in the same TwiML, the implicit-consent ledger row written; tenant B's call from the same caller number writes B's own row (T2). The audit leg landed with #1136. "Media Streams consumes no audio until it has played" is not exercised hermetically (Gather path) → **4 (T2)** |
| **2.3** | **As M**, I want a known customer recognised by their number and a stranger turned into a lead, so nothing falls on the floor | **Given** an inbound number, **when** it matches a stored customer in E.164, **then** they are identified — **and** a non-NANP caller sharing the last 10 digits is **not** matched | **5** ↑ (T2) — a known caller is identified on their timeline, a non-NANP caller sharing the last 10 digits is not matched, a stranger becomes exactly one audited lead | **D:** `identify-caller.test.ts`. *The voice unknown→lead leg is untested at real DB; only the SMS caller is* · **G1 2026-09-12:** `identify-caller.test.ts` 4/4 ✓, single tenant, no audit event asserted → 3 · **#1014-A 2026-09-12:** the voice unknown→lead leg at real Postgres: a stranger's call creates the `leads` row with `lead.created` audit read back through `PgAuditRepository`; a known number resolves the customer; T1 (a number known to tenant B is a stranger to tenant A). `EXTERNAL_TEST_DB_URL=<kept container> RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts <the 9 touched integration files>` → 9 files, 32/32 ✓ at real Postgres → 4 · **§8.2 rung-5 lane (Fable gate on PR #1148 @ 1bb403231, 2026-09-13 20:23Z; merged fc267cdb4):** `telephony-capture-2-3.spec.ts` → 4 passed; the same number is a known customer of tenant B and a stranger to tenant A in one run (T2) → **5 (T2)** |
| **2.4** | **As M**, I want a stranger on the phone to be unable to reach owner-only capability, so my phone line isn't an admin console | **Given** a caller-surface session, **when** the classifier returns an off-profile intent, **then** it becomes `unknown` and is **audited**, never silently dropped | **5** ↑ (T2) — a stranger's off-profile money request is intercepted to `unknown` and audited; the owner's own line is not intercepted; tenant B's owner is a stranger to A | Fixture-proven only. **S:** `grep -rln "intent_off_surface" packages/api/test/integration/` → **empty** · **G1 2026-09-12:** `intent_off_surface` in 0 integration files, 9 unit/src files — fixture-proven, 3 stands · **#1014-B 2026-09-12:** D: `integration/stranger-owner-capability.test.ts` (new) — driven as Twilio drives it (`handleInbound` → `ask_caller` turn → classifying turn) with real `PgUserRepository`/`PgSettingsRepository`/`PgAuditRepository`/`PgProposalRepository` and the real membership loader; only the LLM gateway is scripted: an unmatched caller-ID resolves no actor and lands on the `caller` profile (`phone-actor.ts:62`, `twilio-adapter.ts:1162`); a spoken `send_invoice` from that stranger is intercepted to `unknown`/`intent_off_surface` **before routing** (`intent-classifier.ts:655`, applied at `:2875`) with the audit row `voice.intent_off_surface {intent: send_invoice, profile: caller, confidence: 0.96}` read back via `findByEntity`, and **zero** proposals; **positive control** — the same intent on the tenant's own owner line is not intercepted; an owner-grade lookup from the stranger is refused by the D-026 RBAC (`phone-lookup-surface.ts:168`), a different layer. T1 two ways: tenant B's `owner_phone` and its owner's mobile are both strangers to tenant A while both resolve on B's own line. `RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts test/integration/stranger-owner-capability.test.ts` → 5/5 ✓ (four RED rounds first). `phone-lookups-shared-dispatch.test.ts` already proved the lookup-RBAC legs — the printed *prose only* was wrong. 🚨 closed; PROVEN-REAL-DB, T1 → **4**; the phone-surface leg cannot be hermetic (the classification needs a live model) (PR #1054) · **§8.2 rung-5 lane (Fable gate on PR #1148 @ 1bb403231, 2026-09-13 20:23Z; merged fc267cdb4):** `telephony-capture-2-4.spec.ts` → 3 passed (positive control + T2 with divergent owner phones) → **5 (T2)** |
...
| **2.10** | **As M**, I want an unclaimed text to become a thread I can answer, so SMS isn't a black hole | **Given** concurrent inbound texts from one unmatched number, **when** captured, **then** they collapse to a single open thread with no cross-tenant bleed | **5** ↑ (T2) — two concurrent texts from one unclaimed number collapse to one open thread; the same number texting tenant B never bleeds into A | **D:** `inbound-sms-capture.test.ts` · **G1 2026-09-12:** `inbound-sms-capture.test.ts` 7/7 ✓, **T1** ("does not bleed a captured thread across tenants" — phrasing the falsifier's token list misses), no audit event asserted → 4− · **#1014-A 2026-09-12:** `inbound-sms-capture.test.ts` now reads `sms.inbound.captured` back through `PgAuditRepository` (T1 kept). `EXTERNAL_TEST_DB_URL=<kept container> RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts <the 9 touched integration files>` → 9 files, 32/32 ✓ at real Postgres → 4 · **§8.2 rung-5 lane (Fable gate on PR #1148 @ 1bb403231, 2026-09-13 20:23Z; merged fc267cdb4):** `telephony-capture-2-10.spec.ts` → 2 passed → **5 (T2)** |
| **2.11** | **As M**, I want the AI to refuse to quote a firm price or haggle, so it never commits me to a number I'd lose money on | **Given** a price-pressure turn, **when** the guardrail fires, **then** it speaks the holding line, mints exactly one owner callback, stays in state, and is idempotent when already flagged | **4** ↑ (T3) — reached on SMS: a below-floor discount ask is refused and the owner is routed a counter at the floor; tenant B's looser floor governs B; the voice holding-line FSM leg needs a model (#1119-class) | **All unit.** The one integration file tests the context read, not the guardrail · **G1 2026-09-12:** negotiation-invariant + discount-evaluator + negotiation-guardrail 59/59 ✓ (all in-memory) · **#1014-A 2026-09-12:** the REFUSE branch at real Postgres (`negotiation-guardrail.test.ts`, lane-authored): an out-of-policy ask is refused and routed to the owner with `negotiation_guardrail.sms_routed` audit rows read back; T1. The ALLOW branch is proven by #1012 (PR #1035, same file name — merged at rebase). `EXTERNAL_TEST_DB_URL=<kept container> RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts <the 9 touched integration files>` → 9 files, 32/32 ✓ at real Postgres → 4; pricing code untouched · **§8.2 rung-5 lane (Fable gate on PR #1148 @ 1bb403231, 2026-09-13 20:23Z; merged fc267cdb4):** `telephony-capture-2-11.spec.ts` → 2 passed (one an expected-fail pin); per-tenant floor = T3. The spoken holding line + "stays in state" (`transitions.ts:401-415`, needs classifier intent `negotiation`) → pinned → **4 (T3)** |
| **2.12** | **As M**, I want a property manager's call to be handled as a portfolio account, so my biggest customer isn't treated like a stranger | **Given** a caller resolving to `property_manager`, **when** the call runs, **then** the outcome is **observably different** from a residential call — prompt, priority, and proposal context | **4** (T3) — on the Gather transport a property-manager call carries PRIORITY account context on its proposal and the account section in its classify prompt (integration); `accountType` is settable through the customers route (#1155 fixed); **STORY NOT MET** — nothing reads `accountContext` in web or mobile and nothing routes on `priority`, so the owner sees no difference (#1252) | 🚨 **Assembled and read nowhere.** `session.b2bAccountContext` is written once (`twilio-adapter.ts:953`) and has **no reader**; `buildAccountContextPromptSection` has **zero production callers**. Nothing routes on `ctx.priority`, so the criterion's "observably different" is not delivered by anything — which is rung 2's definition, not 3's. *An earlier draft said the context is "read by one supervisor check." It is not: the supervisor's `resolveAccountType` (app.ts:6858) re-reads `customer.accountType` from the customer row via the proposal's `customerId`. That path is real but independent — it would behave identically if call-time B2B recognition did not exist, so it is not evidence that this is wired.* **S:** `grep -rn "buildAccountContextPromptSection" packages/api/src` → **definition only** · **G1 2026-09-12:** confirmed — `b2bAccountContext` writer 1 / reader 0; `buildAccountContextPromptSection` callers 0 · **#1010 2026-09-12:** wired — `create-voice-turn-processor.ts` resolves `session.b2bAccountContext` → `buildAccountContextPromptSection` → `classifyIntent({ b2bAccountPromptSection })` (separate labelled system message ahead of the transcript). `npx vitest run test/ai/orchestration/intent-classifier-b2b-account-wire.test.ts test/ai/voice-turn/voice-turn-processor.test.ts` → 2/2 + 62/62 ✓. PROVEN-UNIT → 3; the phone surface has no hermetic Playwright path, so 5 needs the §8.2 move ticket · **#1014-A 2026-09-12:** `b2b-account-context-voice.test.ts` — with customers in real Postgres, a `property_manager` caller's session gets the PRIORITY classify-prompt section naming its managed properties, the same tenant's residential caller gets none, and tenant B (no B2B account) gets none — T3; the log line "inbound call: B2B account context assembled" confirms the assembly ran. Read-side proof (no mutation to audit). `EXTERNAL_TEST_DB_URL=<kept container> RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts <the 9 touched integration files>` → 9 files, 32/32 ✓ at real Postgres → 4 · **§8.2 rung-5 lane (Fable gate on PR #1148 @ 1bb403231, 2026-09-13 20:23Z; merged fc267cdb4):** `telephony-capture-2-12.spec.ts` → 2 passed (one an expected-fail pin): property-manager, residential and no-account callers all reach assembly without error; `session.b2bAccountContext` is read only on the media-streams path (`create-voice-turn-processor.ts:4391`), never on Gather; no route sets `accountType` → **#1155** · **Re-grade at main (Fable, origin/main @ 30a3a9627, 2026-09-15 19:07Z):** `RLS_RUNTIME_ROLE=true EXTERNAL_TEST_DB_URL=<fresh pgvector/pgvector:pg16> npx vitest run --config vitest.integration.config.ts test/integration/phone-entity-ambiguous.test.ts test/integration/assistant-photo-estimate-draft.test.ts test/integration/en-route-notice-delivery.test.ts test/integration/availability-buffer-default-label.test.ts test/integration/b2b-account-context-gather.test.ts test/integration/b2b-account-context-voice.test.ts test/integration/customer-account-type-route.test.ts` → 7 files, 14/14 ✓ — `b2b-account-context-gather` 1/1 ✓ (over `handleInbound` → `handleGather` with real Pg repositories, only the property-manager caller's classify request carries the account-context system message and only its proposal carries `accountContext`; tenant A's residential caller and tenant B get neither), `b2b-account-context-voice` 2/2 ✓ (media streams gets the PRIORITY section; a real Gather turn carries the same section), `customer-account-type-route` 1/1 ✓ (POST + PUT round-trip `accountType`, an unknown value → 400 with no write, the edit is audited, tenant B untouched). Rows: 1 of 3 proposals carries `{"priority": true, "accountType": "property_manager", "managedPropertyCount": 1}`; one `customer.updated` `{"changes": ["accountType"]}`. `CLERK_DEV_HMAC_TOKENS=true DB_SSL=false E2E_USE_TEST_DB=true DATABASE_URL=<fresh testcontainer> PORT=<dedicated> E2E_API_URL=… PUBLIC_API_URL=… VITE_API_URL=… E2E_WEB_PORT=<dedicated+1> E2E_DEV_AUTH=0 E2E_NOAUTHBYPASS=0 npx playwright test e2e/telephony-capture-2-12.spec.ts --project=chromium --retries=0 --workers=1` (+ Twilio signing env, api port 38590) → 2 passed ×2 (11.4s, 10.9s), incl. "#1155 — the property-manager call is OBSERVABLY DIFFERENT … (formerly pinned KNOWN GAP)"; `accountType` is set through `POST /api/customers` (persisted `property_manager`, no SQL). Rows (pass 2, TRUNCATE blocked before teardown): for identical emergency words the PM call's `emergency_dispatch` carries `{"priority": true, "accountType": "property_manager", "managedPropertyCount": 0}`; the residential and tenant B `emergency_dispatch` rows carry none. The prompt clause is proven at the integration seam — the hermetic mock scripts classification from the last user message. The `CustomerEdit` account-type control is unit-pinned, not browser-driven. No screenshot: phone surface. Residual: no web or mobile code reads `sourceContext.accountContext` and nothing routes on `priority`, so the difference is persisted but not yet shown in the owner's inbox. Reached; three differently-shaped callers in one run (T3) → **5 (T3)** **Grading-review correction (2026-09-15, #1247 review):** not 5 — `git grep -n "accountContext" 30a3a9627 -- packages/web/src packages/mobile/src` → 0 hits and nothing acts on `priority`, so the persisted difference is not observable to M → 4 (T3), STORY NOT MET (**#1252**). |
...
| # | User story | Acceptance criterion | Rung | Confirm |
|---|---|---|---|---|
| **3.1** | **As J**, I want a call to produce a *proposed* booking, not a booking, so the AI never puts something on my calendar I didn't agree to | **Given** a spoken booking request, **when** the pipeline runs, **then** free text → real resolver → real drafting task → approval → production executor, and **zero appointment rows exist before approval** | **4** ↑ (T2) — phone surface reached to the confirm readback; the confirm turn needs a model (#1119) | **D:** `voice-inbound-appointment.test.ts` · **G1 2026-09-12:** same file as 2.1 — T0, no reachability run → 3 · **#1015 2026-09-12:** `voice-inbound-appointment.test.ts` gains a genuine T1 (tenant B's booking never blocks tenant A's availability; lane A of #1014 added T2 in the same file). `EXTERNAL_TEST_DB_URL=<kept container> RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts <the 9 touched integration files>` → 9 files, 47 passed + 1 expected fail at real Postgres → 4 · **§8.3 phone lane (Fable gate) 2026-09-13:** `e2e/telephony-book-3-1-proposed-booking.spec.ts` — a caller on tenant A's DID, self-signed with A's own subaccount credential through the real `/api/telephony/voice` → `/gather`, reaches the `create_appointment` readback deterministically ("Just to confirm — create appointment. Is that right?") via `matchNewBookingPhrase`, and at that point **zero** `proposals` and **zero** `appointments` rows exist — the row's "never silently books" claim proven the strong way; tenant B's identical call reaches its own readback and neither tenant's state is perturbed (T2). `DATABASE_URL=<testcontainer> DB_SSL=false E2E_DEV_AUTH=0 E2E_USE_TEST_DB=true TWILIO_ACCOUNT_SID=… TWILIO_AUTH_TOKEN=… TWILIO_FROM_NUMBER=… TWILIO_DEFAULT_TENANT_ID=… TENANT_ENCRYPTION_KEY=<64 hex> PUBLIC_API_URL=http://localhost:3000 npx playwright test <spec> --project=chromium --retries=0` → 2 passed (18.6s) at real Postgres. **Not reachable hermetically:** the confirmed proposal, its approval and the appointment + audit row — `confirmIntent` classifies the caller's yes/no through the model gateway and the hermetic no-key gateway answers every yes/no with `unknown` → decision **#1119** |
| **3.2** | **As J**, I want offered times to respect my actual working day, so I'm not booked at 7pm | **Given** my configured hours, **when** slots are generated, **then** only in-hours slots are offered, the buffered booked window is removed, and tenant A's calendar never blocks tenant B | **5** ↑ (T2) — the owner's own hours and buffer shape the offered slots on the real board; tenant A's calendar never blocks tenant B | **D:** `dispatch-availability.test.ts`. 🚨 **DST, per-day hours, technician hours and time-off are unit-only** — the story is broader than the proof · **G1 2026-09-12:** `dispatch-availability.test.ts` 3/3 ✓ — **T2**: "does not let tenant A's appointment block tenant B's availability" (`tenantA`/`tenantB` both seeded); read-only, no mutation to audit. The unit-only half stays 3 · **#1015 2026-09-12:** the V17 business-hours/buffer unit half moved to real Postgres in `dispatch-availability.test.ts`; T2 kept. `EXTERNAL_TEST_DB_URL=<kept container> RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts <the 9 touched integration files>` → 9 files, 47 passed + 1 expected fail at real Postgres — 4 (T2) confirmed · **§8.3 in-app rung-5 lane (Fable gate at origin/main 60ec5a9ca, 2026-09-14 00:08–00:17Z; PR #1150 merged 3d87dd9a4):** fresh test DB + fresh api/vite pair per spec, `DEV_AUTH_BYPASS=true CLERK_DEV_HMAC_TOKENS=true` (the technician-token specs need the second flag; the lane's documented api command omits it). `e2e/journeys/dispatch-availability-3-2-3-3.spec.ts` → 1 passed → **5 (T2)** |
| **3.3** | **As T**, I want to be told when the times I'm being offered are just defaults, so I don't discover my hours were wrong after a customer books | **Given** a tenant with no configured hours, **when** availability returns, **then** hours, buffer and timezone are each labelled as defaults | **4** ↑ (T3) — at the availability API a real onboarded tenant's hours, buffer and timezone are each labelled defaults, and a configured neighbour gets its own labels in the same run (#1158 fixed); **STORY NOT MET** — no client renders the labels, so T is never told (#1243) | In-memory supertest only — **and the web app never calls this endpoint**; only mobile does · **G1 2026-09-12:** `availability-route.test.ts` 9/9 ✓ (in-memory supertest); web never calls the endpoint — mobile only · **#1015 2026-09-12:** new `dispatch-availability-stale-defaults.integration.test.ts` — the stale-defaults disclosure is proven at real Postgres with T1. `EXTERNAL_TEST_DB_URL=<kept container> RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts <the 9 touched integration files>` → 9 files, 47 passed + 1 expected fail at real Postgres → 4 · **§8.3 in-app rung-5 lane (Fable gate at origin/main 60ec5a9ca, 2026-09-14 00:08–00:17Z; PR #1150 merged 3d87dd9a4):** fresh test DB + fresh api/vite pair per spec, `DEV_AUTH_BYPASS=true CLERK_DEV_HMAC_TOKENS=true` (the technician-token specs need the second flag; the lane's documented api command omits it). same spec (3.3 leg): stale/default labels on the real surface; `job_buffer_minutes` is `NOT NULL DEFAULT 30` and written at signup, so `bufferSource` is always `tenant` for an onboarded tenant → **#1158** → **4 (T1)** · **Re-grade at main (Fable, origin/main @ 30a3a9627, 2026-09-15 19:07Z):** `RLS_RUNTIME_ROLE=true EXTERNAL_TEST_DB_URL=<fresh pgvector/pgvector:pg16> npx vitest run --config vitest.integration.config.ts test/integration/phone-entity-ambiguous.test.ts test/integration/assistant-photo-estimate-draft.test.ts test/integration/en-route-notice-delivery.test.ts test/integration/availability-buffer-default-label.test.ts test/integration/b2b-account-context-gather.test.ts test/integration/b2b-account-context-voice.test.ts test/integration/customer-account-type-route.test.ts` → 7 files, 14/14 ✓ — `availability-buffer-default-label` 1/1 ✓: after the real signup bootstrap the buffer is labelled `default` (30 min still applied); PUT identity with a buffer relabels it `tenant` and the slots are unchanged; T2 — tenant B (buffer 0) gets its own label and slots. Rows: `tenant_settings.job_buffer_minutes` 30 (A after the PUT) / 0 (B). `CLERK_DEV_HMAC_TOKENS=true DB_SSL=false E2E_USE_TEST_DB=true DATABASE_URL=<fresh testcontainer> PORT=<dedicated> E2E_API_URL=… PUBLIC_API_URL=… VITE_API_URL=… E2E_WEB_PORT=<dedicated+1> E2E_DEV_AUTH=0 E2E_NOAUTHBYPASS=0 npx playwright test e2e/journeys/dispatch-availability-3-2-3-3.spec.ts --project=chromium --retries=0 --workers=1` → 1 passed ×2 (17.8s, 10.0s): the cold tenant, onboarded through the real Clerk webhook and queried through its own session, gets `timezoneSource`/`businessHoursSource`/`bufferSource` = `default`, `bufferMinutes` 30 and the "Travel buffer not configured" note; configured tenant A gets all three as `tenant` (`Etc/UTC`); the cold timezone is `America/New_York`, not A's. Rows (pass 2, TRUNCATE blocked before teardown): cold tenant `job_buffer_minutes` NULL and `timezone` NULL; A and B `30` / `Etc/UTC`. Reached, T2 (3.2's neighbour) and T3 (a configured and an unconfigured tenant each correct in one run) → the API contract earns **5 (T3)**. No screenshot exists: no SPA route renders this endpoint. The story's "told" is not met on any screen: mobile's `AvailabilityResponse` (`packages/mobile/src/api/appointments.ts:24`) has no `config`, its two callers read `res.slots` only, and web never calls the endpoint → **#1243** **Grading-review correction (2026-09-15, #1247 review):** not 5 — §8.0 reserves 5 for the persona being served (its 9.6 example, API-only with no UI, is held at 4); the API and the screen are two layers of one path here, not two user paths, so the split-cell precedent does not apply → 4 (T3), STORY NOT MET. |
| **3.4** | **As M**, I want a booking POST to be unable to take a slot the calendar wouldn't have offered, so the public page can't be gamed | **Given** an out-of-hours slot, **when** a POST attempts it, **then** the write-side twin refuses | **5** ↑ (T2) — a crafted out-of-hours POST that bypasses the public picker is refused by the write-side twin | **Unit only** · **G1 2026-09-12:** `booking-availability.test.ts` 22/22 + `portal-booking.test.ts` 30/30 ✓ (unit) · **#1015 2026-09-12:** new `public-booking-held-slot.integration.test.ts` drives the real `createPublicBookingRouter` + `PgTenantTransactionRunner`: a real hold row blocks a second booking POST for the same tenant with an audited winner; a neighbour tenant's hold on the identical instant does not block this tenant (T1). `EXTERNAL_TEST_DB_URL=<kept container> RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts <the 9 touched integration files>` → 9 files, 47 passed + 1 expected fail at real Postgres → 4 · **§8.3 in-app rung-5 lane (Fable gate at origin/main 60ec5a9ca, 2026-09-14 00:08–00:17Z; PR #1150 merged 3d87dd9a4):** fresh test DB + fresh api/vite pair per spec, `DEV_AUTH_BYPASS=true CLERK_DEV_HMAC_TOKENS=true` (the technician-token specs need the second flag; the lane's documented api command omits it). `public-booking-out-of-hours-3-4.spec.ts` → 1 passed, T2 → **5 (T2)** |
| **3.5** | **As J**, I want a slot held while I decide, and released if I don't, so a second prospect gets a real answer | **Given** a 24h hold, **when** it expires, **then** the reaper cancels it, clears the flag, emits audit, spares live holds, and a second sweep is a no-op — and an expired hold **stops blocking** the slot | **5** ↑ (T2) — the owner sees the hold badge; the reaper cancels the expired hold, clears the flag, audits, spares live holds, a second sweep is a no-op, the slot re-opens | **D:** `hold-reaper.test.ts`, `slot-conflict-checker.test.ts` · **G1 2026-09-12:** `hold-reaper.test.ts` 2/2 ✓ (audit ✓) + `slot-conflict-checker.test.ts` 4/4 ✓; **T4** via the `hold-reaper sweep` entry in `sweep-tenant-fanout.test.ts`. 4 confirmed · **#1015 2026-09-12:** `hold-reaper.test.ts` gains the T2 hold-visibility assertion (a reaped hold in tenant A never frees or hides a slot in tenant B); T4 entry cited. `EXTERNAL_TEST_DB_URL=<kept container> RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts <the 9 touched integration files>` → 9 files, 47 passed + 1 expected fail at real Postgres — 4 (T2·T4) confirmed · **§8.3 in-app rung-5 lane (Fable gate at origin/main 60ec5a9ca, 2026-09-14 00:08–00:17Z; PR #1150 merged 3d87dd9a4):** fresh test DB + fresh api/vite pair per spec, `DEV_AUTH_BYPASS=true CLERK_DEV_HMAC_TOKENS=true` (the technician-token specs need the second flag; the lane's documented api command omits it). `hold-reaper-3-5.spec.ts` → 1 passed (the public-booking rate limiter is in-memory — fresh api per spec) → **5 (T2)** |
| **3.6** | **As M**, I want it to be *impossible* to double-book Carlos, so I never send one truck to two houses | **Given** two concurrent assignments for the same tech and slot, **when** both race, **then** exactly one succeeds — enforced by a DB `EXCLUDE` constraint, not application code | **5** ↑ (T2) — two concurrent `POST /api/jobs` for the same technician and overlapping slot: exactly one succeeds (DB `EXCLUDE`); tenant B's own race is unaffected | **D:** `technician-double-booking-race.test.ts` — **the strongest capability row in the product** · **G1 2026-09-12:** `technician-double-booking-race.test.ts` 5/5 ✓ — the real `EXCLUDE` race, proven for one tenant with no audit event asserted → 3 until a neighbour and the audit leg are asserted · **#1015 2026-09-12:** `technician-double-booking-race.test.ts` gains the audit read-back and a neighbour tenant (a neighbour's technician is not the one being double-booked — T1). `EXTERNAL_TEST_DB_URL=<kept container> RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts <the 9 touched integration files>` → 9 files, 47 passed + 1 expected fail at real Postgres → 4 · **§8.3 in-app rung-5 lane (Fable gate at origin/main 60ec5a9ca, 2026-09-14 00:08–00:17Z; PR #1150 merged 3d87dd9a4):** fresh test DB + fresh api/vite pair per spec, `DEV_AUTH_BYPASS=true CLERK_DEV_HMAC_TOKENS=true` (the technician-token specs need the second flag; the lane's documented api command omits it). `technician-double-booking-race-3-6.spec.ts` → 1 passed ×2 (the harness needs `CLERK_DEV_HMAC_TOKENS=true`; without it the technician's `/api/me` has no internal id). Related: concurrent first jobs collide on `job_number` (#1152) → **5 (T2)** |
| **3.7** | **As M**, I want the AI to ask instead of guessing when two customers share a name, so it never books the wrong Henderson | **Given** duplicate display names, **when** resolution runs, **then** the result is an ambiguity carrying **both** candidates — never a silent pick | **5** ↑ (T2) on the Gather transport — two same-named customers make the call ask, the caller's answer resolves it, the owner-line `create_job` drafts only the chosen customer (#1118 fixed); booking not driven / media-streams transport **unproven at main** (unit test only, not run) / **4** (T1) in-app | **D:** `entity-resolution.test.ts`, `cancel-appointment-voice.test.ts` · **G1 2026-09-12:** `entity-resolution.test.ts` 94/94 ✓ (**T1** — eight "across tenants" cases) + `cancel-appointment-voice.test.ts` 4/4 ✓ (audit ✓, **T1** `:268`); rung 5 had no reachability run → 4 · **§8.3 phone lane (Fable gate) 2026-09-13:** `e2e/telephony-book-3-7-disambiguation.spec.ts` — two same-named customers on tenant A (real customers API), the resolver genuinely reports ambiguity at real Postgres, and the live call through the real routes proceeds straight to the ordinary confirm readback with **no disambiguation question** and no proposal; pinned as a passing GAP characterization + a `test.fail()` for the desired behaviour; a same-named customer on tenant B is never a candidate (T1). `DATABASE_URL=<testcontainer> DB_SSL=false E2E_DEV_AUTH=0 E2E_USE_TEST_DB=true TWILIO_ACCOUNT_SID=… TWILIO_AUTH_TOKEN=… TWILIO_FROM_NUMBER=… TWILIO_DEFAULT_TENANT_ID=… TENANT_ENCRYPTION_KEY=<64 hex> PUBLIC_API_URL=http://localhost:3000 npx playwright test <spec> --project=chromium --retries=0` → 3 passed (17.4s) incl. the pin. Cause verified in source: `entity_ambiguous` is dispatched only by the in-app adapter; both telephony transports fold the resolver's multi-candidate result into `entity_resolved` → **#1118**. The in-app grade stands; the phone half is not met · **Re-grade at main (Fable, origin/main @ 30a3a9627, 2026-09-15 19:00Z):** `RLS_RUNTIME_ROLE=true EXTERNAL_TEST_DB_URL=<fresh pgvector/pgvector:pg16> npx vitest run --config vitest.integration.config.ts test/integration/phone-entity-ambiguous.test.ts test/integration/assistant-photo-estimate-draft.test.ts test/integration/en-route-notice-delivery.test.ts test/integration/availability-buffer-default-label.test.ts test/integration/b2b-account-context-gather.test.ts test/integration/b2b-account-context-voice.test.ts test/integration/customer-account-type-route.test.ts` → 7 files, 14/14 ✓ — `phone-entity-ambiguous` 5/5 ✓: through the real `createApp()` with signed `/api/telephony/voice` → `/gather` the turn asks (TwiML `<Say>`) and the `entity_ambiguous` audit names exactly tenant A's two candidates; "the one on 12 Oak Street" resolves; `handleGather` B1 drafts `create_job` carrying that customer. Rows: 3 `agent.calling.entity_resolution.entity_ambiguous`, all tenant A, 0 on tenant B; the one `create_job` draft carries tenant A's 12 Oak Street Jamie Rivera (not 48 Pine Avenue, not tenant B's same-address namesake). `CLERK_DEV_HMAC_TOKENS=true DB_SSL=false E2E_USE_TEST_DB=true DATABASE_URL=<fresh testcontainer> PORT=<dedicated> E2E_API_URL=… PUBLIC_API_URL=… VITE_API_URL=… E2E_WEB_PORT=<dedicated+1> E2E_DEV_AUTH=0 E2E_NOAUTHBYPASS=0 npx playwright test e2e/telephony-book-3-7-disambiguation.spec.ts --project=chromium --retries=0 --workers=1` (+ Twilio signing env, api port 38580) → 3 passed ×2 (20.8s, 9.6s; the old `test.fail()` pin is flipped): "more than one record under that name", no proposal, "The first one" → the create_job readback; rows (pass 2, TRUNCATE blocked before teardown): tenant A 2 `entity_ambiguous` + 2 Jamie Riveras, tenant B 0 `entity_ambiguous` + 1 `entity_resolved` + 1 Jamie Rivera, 0 proposals. Graded T2 though the tests say T1: tenant B's own namesake leaves tenant A's candidate count at 2 and tenant A's pair does not make tenant B's call ask — a neighbour's data not changing the answer. Not proven here: the media-streams transport is unit-only (`speechturn-entity-ambiguous.test.ts`, not run); the vehicle is the owner-line `create_job` (no deterministic booking utterance carries a name), so "books" rests on the shared `resolveSchedulingEntities` pipeline, not a driven booking; no in-app run. No screenshot: phone surface → phone **5 (T2)** / in-app **4 (T1)** **Grading-review correction (2026-09-15, #1247 review):** the 5 (T2) is scoped to the Gather transport — "on the phone" over-claimed while media streams is unit-only and no booking is driven. |
...
| **3.9** | **As J**, I want customers reminded the day before, so I stop eating no-shows | **Given** an appointment 24h out, **when** the sweep runs, **then** exactly one reminder sends, durably idempotent | **4** ↑ (T1·T3·T4) — reached: the owner books a job, registers a push device, and the real reminder sweep sends exactly one reminder; 5 pending a neighbour in the reachability run | The **owner push** is real-DB with durable idempotency; **the customer reminder is not** · **G1 2026-09-12:** `appointment-reminder-owner-push.integration.test.ts` 2/2 ✓ but **T0**, no audit event asserted, and the sweep still stubs `listTenantIds: async () => [tenant.tenantId]` (`:171`) with **no entry in `sweep-tenant-fanout.test.ts`** — a tenant-iterating sweep outside the T4 file (like accounting sync), capped at 4 regardless; customer-reminder unit 2/2 ✓ → 3 · **#1015 2026-09-12:** the appointment-reminder sweep now has its `sweep-tenant-fanout.test.ts` entry (real enumerator over 3 tenants, one throwing tenant does not abort the rest, a tenant with nothing due is untouched — T4); `appointment-reminder-owner-push.integration.test.ts` gains the second tenant (T1) and two tenants in two timezones at one instant each getting exactly their own reminder (T3), audit rows read back through `PgAuditRepository`. `EXTERNAL_TEST_DB_URL=<kept container> RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts <the 9 touched integration files>` → 9 files, 47 passed + 1 expected fail at real Postgres. PROVEN-REAL-DB → 4 (tenant-iterating row: T4 met) · **§8.3 in-app rung-5 lane (Fable gate at origin/main 60ec5a9ca, 2026-09-14 00:08–00:17Z; PR #1150 merged 3d87dd9a4):** fresh test DB + fresh api/vite pair per spec, `DEV_AUTH_BYPASS=true CLERK_DEV_HMAC_TOKENS=true` (the technician-token specs need the second flag; the lane's documented api command omits it). `appointment-reminder-sweep-3-9.spec.ts` → 1 passed; no second tenant in the spec (the T3·T4 grades stand from the integration proofs) → **4** reached |
| **3.10** | **As M**, I want to book, move and cancel by talking, so I can do it between attics | **Given** any of the three spoken intents, **when** approved and executed, **then** the real row changes and **exactly one** audit event is emitted per action | **4** (T1) / **3** (T0) — owner line reached to the BOOK readback; MOVE/CANCEL never classify without a model (#1119) | **D:** `voice-inbound-appointment.test.ts`, `reschedule-appointment-voice.test.ts`, `cancel-appointment-voice.test.ts` · **G1 2026-09-12:** cancel 4/4 + reschedule 3/3 ✓ with audit, **T1** (`:268`, `:299`) → move/cancel at 4; the book leg is `voice-inbound-appointment` (T0) → 3; no reachability run backs 5 · **§8.3 phone lane (Fable gate) 2026-09-13:** `e2e/telephony-book-3-10-book-move-cancel.spec.ts` — on the owner's own line (`owner_phone`), BOOK reaches the same `create_appointment` readback as 3.1 with nothing drafted; MOVE and CANCEL stop one turn earlier because neither `reschedule_appointment` nor `cancel_appointment` has any deterministic classify path on this surface (asserted: the classifier never names the intent); a neighbour tenant's identical owner-line calls reach the same frontier independently and tenant A stays untouched (T1). `DATABASE_URL=<testcontainer> DB_SSL=false E2E_DEV_AUTH=0 E2E_USE_TEST_DB=true TWILIO_ACCOUNT_SID=… TWILIO_AUTH_TOKEN=… TWILIO_FROM_NUMBER=… TWILIO_DEFAULT_TENANT_ID=… TENANT_ENCRYPTION_KEY=<64 hex> PUBLIC_API_URL=http://localhost:3000 npx playwright test <spec> --project=chromium --retries=0` → 4 passed (18.4s) at real Postgres. Completing any of the three legs needs a model (**#1119**) |
| **3.11** | **As M**, I want stale schedule proposals to expire, so I'm not approving yesterday's plan | **Given** an unactioned schedule proposal, **when** the TTL passes, **then** it expires and can be re-proposed | **5** ↑ (T2) — a drag-created schedule proposal expires on the real inbox and can be re-proposed with a new window; the neighbour's fresh proposal is spared | **U:** only. 🚨 **Two TTL regimes coexist** — 48h in the worker, 24h default and 4h for `create_appointment` in the guardrail · **G1 2026-09-12:** `proposal-expiry-worker.test.ts` 7/7 + `guardrails-expiration.test.ts` 9/9 ✓ — **but the 24 h / 4 h regime lives in `ai/guardrails/expiration.ts` and has zero production callers**: tested, dead code. The live TTL is the worker's 48 h; "two regimes coexist" overstates it · **dormant-rows lane (Fable gate) 2026-09-12:** D: `integration/proposal-expiry-sweep-3-11.test.ts` (new) — the WORKER's 48 h regime (`SCHEDULE_PROPOSAL_EXPIRY_MS`, `proposals/proposal.ts:109`; `runProposalExpirySweep` registered at `app.ts:6485` over the production `listAllTenantIds`) expires a 50 h-old `create_appointment` (`status = 'expired'` + `proposal.expired` audit row, actor `proposal-expiry-worker`), leaves a `draft_estimate` (no `expiresAt`) and a neighbour tenant's fresh card untouched in the same pass, and lets the operator re-propose with a fresh 48 h window; STRUCTURAL: `ai/guardrails/expiration.ts` has zero runtime importers under `src/` (scanner with a negative control that finds `app.ts` for the worker) — *two regimes* is true as source, false as behaviour. `RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts test/integration/proposal-expiry-sweep-3-11.test.ts` → 5/5 ✓ + 1 expected fail (the desired one-regime state). PROVEN-REAL-DB + STRUCTURAL, T2 → **4**; not T4 (no failure-isolation case; the sweep has no fan-out entry) so 4 is the cap. Retire the dead module → issue **#1078** (PR #1076) · **§8.3 in-app rung-5 lane (Fable gate at origin/main 60ec5a9ca, 2026-09-14 00:08–00:17Z; PR #1150 merged 3d87dd9a4):** fresh test DB + fresh api/vite pair per spec, `DEV_AUTH_BYPASS=true CLERK_DEV_HMAC_TOKENS=true` (the technician-token specs need the second flag; the lane's documented api command omits it). `proposal-expiry-3-11.spec.ts` → 5 of 6 fresh-pair runs passed; the one failure was the dispatch board's first render on a cold vite (15 s locator timeout), not the capability → **5 (T2)**, flake noted |
...
| **4.2** | **As M**, I want dragging a card to *propose* a change, not make one, so a slip of the thumb can't move a customer's appointment | **Given** a drag, **when** it lands, **then** a proposal is created and **the appointment is not mutated** | **5** ↑ (T2) — drag→proposal reached hermetically by both tenants; the audit leg now lands at real Postgres (#1040 fixed by #1128) | `createSchedulingProposal` has **zero** integration coverage. The UI is reachable; the guarantee is not proven · **G1 2026-09-12:** `createSchedulingProposal` in 0 integration files; unit 12/12 ✓ — 3 stands · **#1017 2026-09-12:** new `test/integration/dispatch-drag-proposal.test.ts` — a drag creates a `reschedule_appointment` proposal row (status `draft`) and the appointment row is NOT mutated until approval; T1 (tenant B never sees the proposal, its own appointment untouched). REAL-DB-WRITE-ONLY: product code emits no `proposal.created` audit event for scheduling proposals (issue #1040) — the audit assertion is an `it.skip` with the gap named → 4−, 4 when #1040 lands. `EXTERNAL_TEST_DB_URL=<kept container> RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts test/integration/dispatch-drag-proposal.test.ts test/integration/en-route-sms-keyword.test.ts test/integration/running-late.test.ts test/integration/tech-status-sms.test.ts` → 4 files, 10 passed / 1 skipped at real Postgres · **owner-surfaces lane (Fable gate) 2026-09-12:** rung-5 reachability run — `e2e/journeys/dispatch-drag-proposal.spec.ts`: a real native drag on `/dispatch` creates a `draft` `reschedule_appointment` proposal and the `appointments` row is byte-identical before and after (polled); BOTH tenants drag in the same run, each gets exactly its own proposal in its own inbox and each appointment stays unchanged (T2). `CLERK_DEV_HMAC_TOKENS=true DB_SSL=false DATABASE_URL=<testcontainer> E2E_USE_TEST_DB=true VITE_CLERK_PUBLISHABLE_KEY=pk_test_… npx playwright test <spec> --project=chromium --retries=0` → `dispatch-drag-proposal` 1 passed. Reachability and T2 proven; **5 withheld on the evidence class**: the product emits no `proposal.created` audit event for scheduling proposals (#1040), so the row stays REAL-DB-WRITE-ONLY 4− until that lands (PR #1085) · **#1040 fixed (Fable gate on PR #1128, 2026-09-13 05:38Z):** `createSchedulingProposal` now emits `proposal.created` through the router's wired `ForwardingAuditRepository(PgAuditRepository)` (actor = the dragging user + role, `source: dispatch`, appointment id, proposed window, `skillConstraints`); the `it.skip` in `dispatch-drag-proposal.test.ts` is un-skipped. RED (main's product files, this branch's tests): 3 failed / 2 passed — `expected false to be true` on the audit read-back; GREEN: 5/5 at real Postgres, 12 proposal/feasibility integration files 53 passed / 2 expected fail, unit 2036/2036, web dispatch 109/109, tsc build clean; rows read back: 4 `proposal.created` rows across 2 tenants, each `actor_role=dispatcher`, `skills=none_configured`, `status=draft`. With the write + audit now real and the earlier rung-5 reachability run (PR #1085: both tenants drag on the real `/dispatch`, each gets exactly its own draft proposal, appointments byte-identical) → **5 (T2)** |
| **4.3** | **As M**, I want to see when someone else is dragging the same card, so my wife and I don't fight over the board Saturday morning | **Given** two users, **when** both act, **then** revision tokens order the writes and presence shows who holds which card | **5** ↑ (T2) — revision tokens order the writes at real Postgres; presence at real Redis, seen and cleared in two real browsers | Presence is in-memory/Redis — **structurally incapable** of a real-Postgres proof. Score it honestly and stop calling it 4 · **G1 2026-09-12:** `redis-presence-store.test.ts` 26/26 ✓ (in-memory/Redis); 3 is this row's ceiling unless Redis joins the Docker lane · **#1017 2026-09-12:** stays 3 by architecture — presence is `InMemoryDispatchPresenceStore` / `RedisDispatchPresenceStore` only (`dispatch/presence-store.ts:66,137-149`, `redis-presence-store.ts:53`); `grep presence db/schema.ts` → 0. No DB test written on purpose · **Redis joined the Docker lane (Fable gate on PR #1129, 2026-09-13 05:59Z):** `packages/api/test/integration/dispatch-presence-redis.test.ts` drives the real `RedisDispatchPresenceStore` at a `redis:7-alpine` testcontainer through two independent ioredis connections — take/hold/release seen across connections, real-clock lease expiry, T2 (tenant B's hold on the identical card id never appears under tenant A's key), and the revision-token half at real Postgres (user 1's write advances `updated_at`, user 2's stale `appointmentVersion` → 409 `STALE_APPOINTMENT`, a re-based write succeeds) → 4/4. `e2e/journeys/dispatch-presence.spec.ts` (api webServer with `REDIS_URL` on a second real Redis container): owner 1 drags, owner 2 — invited through the real invitation flow — sees the "is moving this" chip appear and clear, a tenant-B owner on their own board sees nothing WHILE the hold is live → 1 passed (42.9s); the Redis container afterwards holds `dispatch:presence:<tenant>:<date>` keys for both tenants with TTLs. Screenshots `8-4-presence/1-4`. Scaffolding (technician row, cards) seeded over psql like the owner-surfaces fixture; tenant/owners/membership through the real routes. Not proven: cross-replica presence from a browser, the WS-gateway transport (off by default). → **5 (T2)**; the earlier "3 is the ceiling unless Redis joins the Docker lane" is retired |
| **4.4** | **As Carlos (M's tech)**, I want my own day view and nobody else's, so I see my work and not the shop's books | **Given** a technician session, **when** the day loads, **then** a 23:00-local appointment lands on the tenant date and a tech of tenant B is not submittable by tenant A | **5** ↑ (T2) — Carlos's own day view reached hermetically with his own session (no auth bypass); 23:00-local lands on the tenant date; tenant B's technician id refused | **D:** `dispatch-technician-day-window.test.ts`, `technician-location-authz.test.ts` · **G1 2026-09-12:** `technician-location-authz.test.ts` 6/6 ✓, **T1** (`:76` "does not leak across tenants", 7 tenants) + `dispatch-technician-day-window.test.ts` 3/3 ✓ (T0); read/authz, 4 stands at T1 · **owner-surfaces lane (Fable gate) 2026-09-12:** rung-5 reachability NOT earned — `e2e/journeys/technician-day-view.spec.ts` proves the 23:00 America/Los_Angeles appointment lands on the tenant-local day (stored 06:00Z next day) through the owner's session, and that a technician session requesting tenant B's technician id is refused 403; but the technician's OWN day view is not reached: the hermetic api webServer forces `DEV_AUTH_BYPASS=true`, which skips the DB authorization loader, so `canonicalUserId` never resolves for an HMAC-verified technician and his own request 403s too (the page shows "Failed to load appointments"; pinned as a Playwright `test.fail()`; harness follow-up #1086). `CLERK_DEV_HMAC_TOKENS=true DB_SSL=false DATABASE_URL=<testcontainer> E2E_USE_TEST_DB=true VITE_CLERK_PUBLISHABLE_KEY=pk_test_… npx playwright test <spec> --project=chromium --retries=0` → `technician-day-view` 1 passed + 1 expected fail. 4 (T1) stands (PR #1085) · **#1086 closed for this route (Fable gate on PR #1134, 2026-09-13 06:30Z):** a third Playwright project `chromium-noauthbypass` (api :3002 / web :5175, `DEV_AUTH_BYPASS` unset, `CLERK_DEV_HMAC_TOKENS=true`; owner via the real Clerk webhook + HMAC session, Carlos via the real invitation flow + HMAC session) lets `app.ts` wire the DB authorization loader, so `canonicalUserId` resolves and SEC-22 admits Carlos's OWN `GET /api/dispatch/technician/:id/appointments` (200) while still refusing tenant B's technician id (403). `e2e/journeys/technician-day-view.spec.ts --project=chromium-noauthbypass` → 2 passed (1.0m): the former `test.fail()` pin passes; Carlos sees only his day in the real browser incl. the 23:00 America/Los_Angeles appointment (stored 06:00Z next day; snapshot `4.4-technician-day-appointment.snapshot.txt`, screenshot `4.4-technician-day-own-appointments.png`); T2. Harness-only, 0 product files → **5 (T2)** |
| **4.5** | **As Carlos**, I want "on my way" to work the same whether I tap, speak, or text it, so I don't have to remember which app | **Given** any of the four entry points, **when** fired, **then** one audited act with a TECH actor plus a customer ETA dispatch row | **5** (T2) — tap (technician day view) and text (OMW keyword) reached hermetically with the same TECH-actor audit + customer ETA dispatch row, which now settles `sent` with its `en_route_notice_sent` analytics row (#1131 fixed); speak/chat proven at the DB (T1) | Voice, phone and chat legs proven. 🚨 **The SMS-keyword leg is unit-only** — the story claims four legs and proves three · **G1 2026-09-12:** `en-route-voice.test.ts` 7/7 ✓ with audit events, **T1** (`tenantB` :91/:125, cross-tenant negative) — voice/phone/chat legs at 4; the SMS-keyword leg is unit-only (141 unit tests across 9 files) → 3 · **#1017 2026-09-12:** the SMS-keyword "OMW" leg now runs at real Postgres through the real `registerEnRouteSmsKeyword` handler (`en-route-sms-keyword.test.ts`): same `appointment.en_route_triggered` audit row + `delay_notice_state` row as the app/voice/chat legs (`en-route-voice.test.ts`); T1. `EXTERNAL_TEST_DB_URL=<kept container> RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts test/integration/dispatch-drag-proposal.test.ts test/integration/en-route-sms-keyword.test.ts test/integration/running-late.test.ts test/integration/tech-status-sms.test.ts` → 4 files, 10 passed / 1 skipped at real Postgres → 4 · **Reachability (Fable gates on PR #1134 tap leg + PR #1130 text leg, 2026-09-13 06:30Z):** tap — `e2e/journeys/on-my-way-tap.spec.ts --project=chromium-noauthbypass` → Carlos's own session taps "on my way" in the real day view: `appointment.en_route_triggered` with `actor_role=technician` + a `delay_notice_state` row (snapshots `4.5-en-route-audit-a`, `4.5-delay-notice-state-a`, screenshot `4.5-on-my-way-tap-notified.png`); text — `e2e/telephony-omw-keyword-4-5.spec.ts` (signed inbound SMS, dedicated port) → 4 passed: bare `OMW` and the phrase form fire the same audited act + dispatch row, an unregistered number does nothing, tenant B's tech fires its own rows only. **#1131 on this path:** the delivery worker sends the customer notice, then its `dispatch_analytics` insert violates the CHECK (`en_route_notice_sent` not allowed) and the catch marks `delay_notice_state.status = failed` — delivered notices recorded as failed, duplicate-send risk (Josh's queue). Speak/chat legs: `en-route-voice.test.ts` at the DB (T1), not reached hermetically → **5 (T2)** with both residuals on the row · **Re-grade at main (Fable, origin/main @ 30a3a9627, 2026-09-15 19:06Z):** `RLS_RUNTIME_ROLE=true EXTERNAL_TEST_DB_URL=<fresh pgvector/pgvector:pg16> npx vitest run --config vitest.integration.config.ts test/integration/phone-entity-ambiguous.test.ts test/integration/assistant-photo-estimate-draft.test.ts test/integration/en-route-notice-delivery.test.ts test/integration/availability-buffer-default-label.test.ts test/integration/b2b-account-context-gather.test.ts test/integration/b2b-account-context-voice.test.ts test/integration/customer-account-type-route.test.ts` → 7 files, 14/14 ✓ — `en-route-notice-delivery` 1/1 ✓: the real coordinator → `PgQueue` → worker → gated delivery settles the notice `sent`, records one `en_route_notice_sent` and texts once even after a second tap; tenant B stays empty (T1). Rows: 1 `delay_notice_state` `:en_route` `sent`/`sms` with no `last_error`, 1 `dispatch_analytics` `en_route_notice_sent`, 1 `message_dispatches` `appointment_en_route` `sent`. `CLERK_DEV_HMAC_TOKENS=true DB_SSL=false E2E_USE_TEST_DB=true DATABASE_URL=<fresh testcontainer> npx playwright test e2e/journeys/on-my-way-tap.spec.ts --project=chromium-noauthbypass --retries=0 --workers=1` → 2 passed ×2 (32.4s, 47.1s), incl. "#1131 — en-route deliveries record en_route_notice_sent analytics (formerly pinned KNOWN GAP)". Rows (pass 2, TRUNCATE blocked before teardown), each of tenants A and B: 1 `appointment.en_route_triggered` with `actor_role=technician`, 1 `delay_notice_state` `sent`/`sms` with empty `last_error`, 1 `en_route_notice_sent`, 1 `message_dispatches` `sent`; no `failed` state. Snapshots `4.5-en-route-audit-a` / `4.5-delay-notice-state-a` (status `sent`), screenshot `4.5-on-my-way-tap-notified.png`. The #1131 residual is closed at main; the text leg (`telephony-omw-keyword-4-5.spec.ts`) was not re-run; speak/chat remain DB-only (T1) → **5 (T2)**, unchanged |
| **4.6** | **As Carlos**, I want to tell the customer I'm running late in one tap with gloves on, so I don't pull over to type | **Given** the chip row, **when** I tap 10/20/30, **then** **the chip row *is* the confirm** — no second dialog — and a delay notice is written | **4** (T1) — **story now met on the technician surface**: tapping Yes then a delay chip is the one-tap confirm, with no second dialog, and writes the notice and its audit row. Reachability is proven in a real browser; rung 5 needs T2 and the proving spec asserts only T1 | In-memory supertest plus a jsdom test that the chip hits the right endpoint; **no real-DB write proof** · **G1 2026-09-12:** `appointments.running-late.test.ts` 9/9 + `TechnicianDayView.test.tsx` 21/21 ✓ (supertest + jsdom) — 3 stands; the grep for this row must use the hyphenated `running-late` · **#1017 2026-09-12:** `running-late.test.ts` at real Postgres: one tap writes `appointment.running_late_triggered` + a queued, consent-gated `delay_notice_state` row; T1 (a tenant-B technician cannot see or trigger it). **CHANGED (Fable):** the product sends a consent-gated notice directly, it does not raise a comms-class proposal — the acceptance criterion is re-specced to the tested behaviour; the "never auto-sent" clause holds through the consent gate, not through a proposal. `EXTERNAL_TEST_DB_URL=<kept container> RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts test/integration/dispatch-drag-proposal.test.ts test/integration/en-route-sms-keyword.test.ts test/integration/running-late.test.ts test/integration/tech-status-sms.test.ts` → 4 files, 10 passed / 1 skipped at real Postgres → 4 · **Technician surface (Fable gate on PR #1134, 2026-09-13 06:30Z):** `e2e/journeys/running-late-chip.spec.ts --project=chromium-noauthbypass` → 2 passed (one an expected-fail pin): tapping "Yes" then the 20-minute chip in `TechJobView.tsx` (`:1086` "Running behind?", `:1115` `onClick={() => setDelayMinutes(minutes)}`) makes ZERO requests — live network capture, screenshot `4.6-chip-row-no-api-call.png` — so the acceptance ("the chip row is the confirm … a delay notice is written") is not met on the surface the story names; the wired path is the GPS-lateness prompt in `TechnicianDayView.tsx:556` (Yes → `POST /api/appointments/:id/running-late`). The API-level proof (`running-late.test.ts`: `appointment.running_late_triggered` + consent-gated `delay_notice_state`, T1) stands → **4 (T1)** with the surface marked NOT MET; **#1135** holds the fix (wire the chip to the route with `delayMinutes`) · **Re-grade at main (Fable, origin/main @ 792f8d2b9, 2026-09-14 18:02Z):** `e2e/journeys/running-late-chip.spec.ts --project=chromium-noauthbypass` (real Postgres, no DEV_AUTH_BYPASS, technician session) → 1 passed: the 20-minute chip writes `appointment.running_late_triggered` + `delay_notice_state`, tenant B's running-late audit count is 0 (screenshot `4.6-chip-row-one-tap-confirm.png`); `test/integration/running-late.test.ts` → 2/2 ✓ (T1). #1135 fixed by #1165 → **4 (T1)**, T2 is the gap to 5 · **Row artifact at main (Fable, origin/main @ 859db9f94, 2026-09-15 19:20Z):** `CLERK_DEV_HMAC_TOKENS=true DB_SSL=false E2E_USE_TEST_DB=true DATABASE_URL=<fresh testcontainer> npx playwright test e2e/journeys/running-late-chip.spec.ts --project=chromium-noauthbypass --retries=0 --workers=1` (teardown TRUNCATE blocked) → 1 passed (39.7s). That test now carries #1193's tenant-B leg (a different chip in a separate session), not re-graded here. Rows: `appointment.running_late_triggered` with `actor_role=technician` ×1 per tenant, `delayMinutes` 20 (7f8083ec) and 10 (78bb3497); `delay_notice_state` 1 per tenant `sent`/`sms` with empty `last_error`; `message_dispatches` `delay_notice` `sent` 1 per tenant; screenshots `4.6-chip-row-one-tap-confirm.png`, `4.6-chip-row-tenant-b-different-chip.png`. Evidence `regrade-evidence3/running-late-chip-1-rows.txt`; grade cell unchanged |
...
| **4.8** | **As M**, I want one tech texting OUT to produce one proposal **per affected customer**, so I can handle the difficult one differently | **Given** a verified tech OUT, **when** processed, **then** an unavailable block, a reschedule proposal **per appointment** each carrying a brand-voiced message, and an audit row — idempotent same-day, and **an OUT from an unregistered number is not actioned** | **5** (T3) — a verified tech OUT through the real signed SMS route yields the block, one reschedule draft per affected appointment and the audit rows; idempotent; unregistered number refused; each draft carries the tenant's brand voice; tenant B with different data and a different timezone gets its own result in the same run | **D:** `tech-status-sms.test.ts` · **G1 2026-09-12:** `tech-status-sms.test.ts` 3/3 ✓ with audit, single tenant → 3 · **#1017 2026-09-12:** `tech-status-sms.test.ts` gains the second-tenant assertion (tenant B's tech OUT routes to tenant B's own block + proposal) with `tech_status.recorded` / `duplicate` / `unverified_mobile` audit rows read back. `EXTERNAL_TEST_DB_URL=<kept container> RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts test/integration/dispatch-drag-proposal.test.ts test/integration/en-route-sms-keyword.test.ts test/integration/running-late.test.ts test/integration/tech-status-sms.test.ts` → 4 files, 10 passed / 1 skipped at real Postgres — 4 confirmed at T1 · **Phone surface (Fable gate on PR #1130, 2026-09-13 06:32Z):** `e2e/telephony-tech-out-4-8.spec.ts` (api on a dedicated port, real Postgres, `x-twilio-signature` over `PUBLIC_API_URL`+path, tenant from the path + integration + AccountSid) → 5 passed ×2 at 23:27 Pacific after the clock-safe fix (tenant A `UTC`, tenant B picked at runtime nearest its own noon): a registered tech's OUT → `unavailable_blocks` row + one `reschedule_appointment` draft per remaining appointment (each with a `draftSms`) + `tech_status.recorded`; a second OUT the same tenant-local day → `duplicate`, no new rows; an OUT from an unregistered number → `unverified_mobile`, nothing actioned; T2/T3: tenant B's own tech/mobile/appointment/timezone produce B's block and proposal, A's counts unchanged. NOT earned: the "brand-voiced message" clause and its per-tenant T3 — the hermetic mock has no `brand_voice_v1` branch so every draft carries the generic string (`test.fail()` pin, **#1132**). Read-back polls after each create are the **#1133** workaround, marked → **5 (T2)** · **Re-grade at main (Fable, origin/main @ 792f8d2b9, 2026-09-14 18:02Z):** `e2e/telephony-tech-out-4-8.spec.ts` → 5 passed, incl. "each reschedule SMS draft carries the tenant's brand voice / business name (#1132 fixed)" (fix #1175) and "T2/T3: tenant B (different data AND a different per-tenant timezone) gets its own OUT block/proposal/audit, and tenant A's rows are unchanged" → **5 (T3)** · **Row artifact at main (Fable, origin/main @ 859db9f94, 2026-09-15 19:19Z):** `e2e/telephony-tech-out-4-8.spec.ts` (phone env, api :38580; a BEFORE TRUNCATE trigger kept the Playwright teardown from emptying the test DB before the dump) → 5 passed (9.6s); rows: `tech_unavailable_blocks` 20a6a2db (tz `UTC`) 2026-09-15 00:00Z→09-16 00:00Z `out`, 2d1ab42c (tz `America/Los_Angeles`) 07:00Z→07:00Z `out`; `reschedule_appointment` `ready_for_review` 20a6a2db ×2 (draftSms "Hi Jamie Rivera, this is Wrench Bros Plumbing 5313 — …", "Hi Morgan Lee, this is Wrench Bros Plumbing 5313 — …"), 2d1ab42c ×1 ("Hi Robin Nguyen, this is Neighbour Plumbing Co 5313 — …"); audit 20a6a2db `tech_status.recorded` 1 / `tech_status.duplicate` 1 / `tech_status.unverified_mobile` 1, 2d1ab42c `tech_status.recorded` 1. Evidence `regrade-evidence3/telephony-tech-out-4-8-1-rows.txt`; grade cell unchanged |
| **4.9** | **As M**, I want the closest certified tech assigned automatically, so I stop doing routing math in my head | **Given** a job needing a skill, **when** assignment runs, **then** only qualified techs are offered | **2** — present, dormant: the empty skill list is now an explicit, audited outcome (#1001 answer, PR #1128); skills-based assignment not built | The whole file is nine lines returning `[]`. 🚨 **And it is wired into `checkFeasibility`, so the empty skill list reads as "always feasible"** rather than as a visible gap · **G1 2026-09-12:** the nine-line stub and its `checkFeasibility` wiring confirmed; Josh (map ticket #1001): make the empty skill list an explicit audited outcome and regrade at 2 · **#1001 answer applied (Fable gate on PR #1128, 2026-09-13 05:38Z):** `FeasibilityResult.skillConstraints: none_configured | evaluated | not_evaluated` (`scheduling/feasibility-types.ts`), `skillMatchIssues` returns the status, the no-technician early return reports `not_evaluated`, and the dispatch creation path persists it on the `proposal.created` audit row — so an empty skill list reads as "no skill constraints configured", not "always feasible". RED on main: unit `feasibility-skill.test.ts` 5 failed / 4 passed (`expected undefined to be 'none_configured'`), integration 3 failed / 2 passed; GREEN: unit 9/9, integration 5/5 (T1 leg: tenant B's own drag writes its own record). Josh's decision on #1001: regrade at **2** (present, dormant) rather than a 0 that masquerades as feasibility; `skillConstraints` reaches no UI yet (`ConfirmProposalDialog.tsx:63`) and lives only in `audit_events.metadata` |
...
| **4.11** | **As Carlos**, I want to be told when I'm assigned to something new, so I'm not surprised at 7am | **Given** an assignment change, **when** it commits, **then** the technician is notified | **5** ↑ (T2) — the owner assigns Carlos on the real board; the assignment row and `appointment.technician_assigned` audit land; push delivery is the external Expo provider | `setTechnicianAssignmentNotifier` has **zero production callers** (it is unit-tested — `assignment-notifications.test.ts`). Every production assignment fires a **silent no-op** — while the module's doc-comment says *"app.ts registers one notifier."* **Carlos is never notified** · **G1 2026-09-12:** `assignment-notifications.test.ts` 21/21 ✓ (unit); `setTechnicianAssignmentNotifier` has zero production callers — 2 stands · **#1010 2026-09-12:** `app.ts` now registers `TechnicianAssignmentNotifier` (push via `notifyUser`, SMS on the owner-class staff path only when a delivery provider exists). `RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts test/integration/assignment-notifier-wiring.test.ts` → 3/3 ✓ at real Postgres: one assignment → one push + one SMS with the `appointment.technician_assigned` audit row read back through `PgAuditRepository`; **T1** — a second tenant's technician is never notified. PROVEN-REAL-DB → 4. Review fix: `notifyUser` now honours per-user mutes like `dispatch()` (782da2f69) · **#1017 2026-09-12:** proof cited, not duplicated: `assignment-notifier-wiring.test.ts` 3/3 (PR #1029) · **Reachability (Fable gate on PR #1134, 2026-09-13 06:30Z):** `e2e/journeys/technician-assignment-notification.spec.ts --project=chromium-noauthbypass` → 1 passed (1.1m): the owner drags + approves an assignment on the real `/dispatch` board and inbox (screenshots `4.11-before-drag.png`, `4.11-after-approve.png`); approval executes on the detached 1-second sweep after the 5 s undo window (the spec polls); at real Postgres the assignment row (`4.11-assignment-row-a.snapshot.txt`) and the `appointment.technician_assigned` audit rows (`4.11-technician-assigned-audit-a.snapshot.txt`) exist for Carlos; T2 — tenant B's assignment notifies only B's technician. The notifier itself is proven at the DB by `assignment-notifier-wiring.test.ts` (PR #1029: one push + one SMS + audit, T1); the push goes to the Expo provider (external, writes no row by design) and SMS only when a delivery provider is wired — recorded, not a shortcut → **5 (T2)** |
...
| **5.2** | **As M**, I want before/after photos attached to the job, so I can defend the invoice | **Given** a captured photo, **when** stored, **then** category and before/after pairing survive the round trip | **4** ↑ (T1) — attach half reached hermetically; pairing not reachable on the named surface (#1122) | **D:** job-photo integration tests · **G1 2026-09-12:** no job-photo integration test exists under `test/integration/`; the proofs are unit — `test/jobs/job-photos.test.ts`, `test/attachments/*.test.ts`, `test/routes/attachments.route.test.ts`, 97/97 ✓ incl. "does not leak another tenant attachments" (in-memory). PROVEN-UNIT · **#1018 2026-09-12:** new `test/integration/job-photo-round-trip.test.ts` — a photo attached through the real attachment/job-photo path lands the `job_photos` row + shadow `attachments` row and the `job.photo.upload_requested` / `job.photo.attached` audit rows read back through `PgAuditRepository`; T1 — a neighbour tenant cannot list, receive, or read the photo, its rows, or its audit event. `EXTERNAL_TEST_DB_URL=<kept container> RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts test/integration/job-photo-round-trip.test.ts test/integration/voice-idempotency.test.ts` → 2 files, 14/14 ✓ at real Postgres. PROVEN-REAL-DB → 4 · **§8.5 reachability lane (Fable gate) 2026-09-13:** `e2e/journeys/job-photo-attach.spec.ts` — a technician session (tenant-scoped HMAC, `CLERK_DEV_HMAC_TOKENS`) opens the real technician job screen (`/jobs/:id?view=tech`), the real camera sheet captures a fake-device frame, the client runs presign → PUT → attach through the real routes; the `job_photos` row + the RV-005 shadow `attachments` row exist, the gallery shows *Photos 1*, and it survives a reload; a neighbour tenant's owner gets `200 []` for the job's photos and 404 attaching tenant A's file (T1). `CLERK_DEV_HMAC_TOKENS=true DB_SSL=false DATABASE_URL=<testcontainer> E2E_USE_TEST_DB=true … npx playwright test e2e/journeys/job-photo-attach.spec.ts --project=chromium --retries=0` → 1 passed (1.2m) at real Postgres, chromium project. **Why not 5:** (1) the criterion's *pairing* clause cannot be reached from the technician screen — `TechJobView` hardcodes every capture as `before` and offers no pairing control; the pairing seam exists only on `/jobs/:id/photos` and the attachments API → **#1122**; (2) the technician's own day view 403s under the hermetic harness (**#1086**), so the lane navigated to the job screen directly; (3) T1 only. The dev storage provider discards the PUT bytes, so the thumbnail is the alt text — the rows are the proof, as documented |
| **5.3** | **As Carlos**, I want to log my hours by talking, so paperwork never follows me home | **Given** a spoken duration, **when** executed, **then** a `time_entries` row lands with the resolved `jobId`, **exactly one** audit event, tenant-scoped — **and it is counted by the job-profit query** | **4** ↑ (T1) — phone route reached; the utterance never classifies without a model (#1119) | **D:** `log-time-entry-execution.test.ts` ✅ *executed* · **G1 2026-09-12:** 4/4 ✓, **T1** (`:217` "does not expose the entry to another tenant") · **§8.5 reachability lane (Fable gate) 2026-09-13:** `e2e/journeys/log-time-by-voice.spec.ts` — a signed inbound call from the technician's registered mobile reaches the real `/api/telephony/*` routes at real Postgres and resolves a session; "log two hours on the Garcia job" does **not** become a `time_entries` row because classification never reaches `log_time_entry` without a live model (RED then GREEN as a characterization); tenant B's technician calling B's DID with the same utterance lands only under B and A's job-profit query is unchanged (T1). Same invocation as 5.2 plus the Twilio env → 3 passed (1.1m). Rung 5 withheld on the same wall as 3.1/3.10 — decision **#1119** |
...
| # | User story | Acceptance criterion | Rung | Confirm |
|---|---|---|---|---|
| **7.1** | **As M**, I want a quote drafted from the call that already happened, so I'm not re-typing what the customer told the AI | **Given** a spoken description **or** a customer photo, **when** drafted, **then** a real estimate/proposal row persists **plus** its audit event | **4** ↑ (T1) customer-photo leg — a chat photo drafts a proposal from the image plus its audit row; a neighbour's fileId is refused (#1144/#1173 fixed) / **4** (T1) spoken-description leg, **not re-confirmed at main** — its test is red at `:234` (#1242); the #1141 "T2" asserts isolation only | **D:** `draft-estimate-execution.test.ts`, `mms-to-quote.int.test.ts` · **G1 2026-09-12:** `draft-estimate-execution.test.ts` 4/4 ✓ with audit, **T1** (`:264`); `mms-to-quote.int.test.ts` 3/3 ✓ (T0). Rung 5 had no reachability run → 4 (MMS leg 3) · **§8.7 rung-5 lane (Fable gate on PR #1141, 2026-09-13 18:54Z):** `e2e/journeys/estimate-chat-draft-7-1-7-3.spec.ts` → 2 passed (one an expected-fail pin): the owner's `/assistant` turn goes through the real `POST /api/assistant/chat` (deterministic pre-LLM draft short-circuit), Approve lands a real `estimates` row + `estimate.created`; T2. The photo leg: attachment bytes never leave `AssistantPage.tsx:69-90` — pinned → **#1144** · **Re-grade at main (Fable, origin/main @ 30a3a9627, 2026-09-15 19:05Z):** `RLS_RUNTIME_ROLE=true EXTERNAL_TEST_DB_URL=<fresh pgvector/pgvector:pg16> npx vitest run --config vitest.integration.config.ts test/integration/phone-entity-ambiguous.test.ts test/integration/assistant-photo-estimate-draft.test.ts test/integration/en-route-notice-delivery.test.ts test/integration/availability-buffer-default-label.test.ts test/integration/b2b-account-context-gather.test.ts test/integration/b2b-account-context-voice.test.ts test/integration/customer-account-type-route.test.ts` → 7 files, 14/14 ✓ — `assistant-photo-estimate-draft` 3/3 ✓: the `draft_estimate` model request carries the tenant-scoped signed image part; the photo-only turn drafts from the photo; tenant B's fileId is refused (no image part, no draft). Rows: 2 `draft_estimate` proposals with line "Repair shown in photo…" and `photoFileIds` = the one `assistant_chat_photo` file, 2 `assistant.photo_estimate_drafted`, one tenant; the neighbour's file has no proposal. `CLERK_DEV_HMAC_TOKENS=true DB_SSL=false E2E_USE_TEST_DB=true DATABASE_URL=<fresh testcontainer> PORT=<dedicated> E2E_API_URL=… PUBLIC_API_URL=… VITE_API_URL=… E2E_WEB_PORT=<dedicated+1> E2E_DEV_AUTH=0 E2E_NOAUTHBYPASS=0 npx playwright test e2e/journeys/estimate-chat-draft-7-1-7-3.spec.ts --project=chromium --retries=0 --workers=1` → **1 failed / 1 passed ×4** (2 passes, each with its retry). The photo test passes every run: files/upload-url → PUT → `POST /api/assistant/chat` with `attachments` → a `ready_for_review` proposal "Repair shown in photo" + one `assistant.photo_estimate_drafted`; tenant A naming B's fileId → "I couldn't open that photo", no draft, B's file 403/404 to A (T2). The spoken test fails every run at `:234` `expect(approveBtn2).toBeDisabled()` → Received: enabled — the 7.2/7.3 uncatalogued leg (**#1242**). Its 7.1 asserts before `:234` passed each run (`estimates` row + `estimate_line_items` catalog/15000 + one `estimate.created`; screenshots `7.1-drafted-proposal-catalog.png`, `7.1-approved-proposal-catalog.png`); its T2 neighbour check after `:234` did not run. Scratch copy outside the repo driving the real Assistant photo picker in the browser → 1 passed (its first two runs failed on a wrong send locator in the scratch itself, not in the product): upload, send, a `ready_for_review` draft "Repair shown in photo" + one audit row (screenshots `7.1-photo-attached.png`, `7.1-photo-drafted-proposal.png`); the draft names no customer — the reply asks for one. Photo leg reached + T2 → **5 (T2)**; the spoken leg keeps **5 (T2)** from the #1141 gate but is not re-confirmed at main until #1242 lands **Grading-review correction (2026-09-15, #1247 review):** not 5 — the photo leg's "T2" block only refuses tenant A naming tenant B's fileId (a neighbour never drafts), which is the rubric's T1 example ("rejects cross-tenant access"); the spoken leg's inherited #1141 "T2" has the same isolation-only shape. Both legs → 4 (T1). |
...
| **7.7** | **As M**, I want a customer who approves an old version to be stopped, so nobody holds me to a price I already revised | **Given** a revised estimate, **when** approval arrives with the stale version, **then** it is **rejected** and the status stays `sent`; with the current version it is accepted | **4** ↑ (T1) — reached: a stale-version approval on the public page is rejected (409 + banner) and the status stays `sent`; the current version is accepted; 5 pending a T2 leg | **A guard deciding which price M is legally bound to has never touched a real database** · **G1 2026-09-12:** PROVEN-UNIT — `estimate-revise-lock`, `public-estimate-service`, `line-item-normalization`, `estimate-reminder-worker` 80/80 ✓ · **#1012 2026-09-12:** new `test/integration/estimate-stale-revision.test.ts` — production `reviseEstimate` bumps `version`; a stale `expectedVersion` accept is refused with the mapped `ConflictError`, no state change, no phantom accept audit alongside the genuine `estimate.revised` row; enforced independently for a second, wholly separate tenant with cross-reads null both ways (T1). `RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts test/integration/estimate-phases.test.ts test/integration/sweep-tenant-fanout.test.ts test/integration/estimate-stale-revision.test.ts test/integration/negotiation-guardrail.test.ts` → 4 files, 36/36 ✓ at real Postgres (`PgAuditRepository` read-back) → 4 · **§8.7 rung-5 lane (Fable gate on PR #1141, 2026-09-13 18:54Z):** `estimate-stale-revision-approve-7-7.spec.ts` → 1 passed; both tenants exercised but not with divergent data → **4 (T1)** reached |
| **7.8** | **As M**, I want exactly one accepted estimate per job, so I can't accidentally have two live prices | **Given** two concurrent approvals on one job, **when** they race, **then** exactly one is accepted — **and the loser gets a clean conflict, not a 500** | **5** ↑ (T2) — two same-instant public approvals: exactly one accepted, the loser a clean 409 CONFLICT (never a 500) | The race **is** genuinely tested against the real partial unique index. 🚨 **The conflict mapping is not** — the test never inspects the rejected settlement · **G1 2026-09-12:** same file — the partial-unique-index race is real but single-tenant with an in-memory audit leg; no test inspects the rejected settlement → 3 · **#1012 2026-09-12:** the concurrent-approval test now inspects the rejected settlement (`ConflictError` matching `/already.*accepted/i`), asserts exactly one `public_estimate.approved` audit row, and that the other tenant's `findByJob` is empty (T1). `RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts test/integration/estimate-phases.test.ts test/integration/sweep-tenant-fanout.test.ts test/integration/estimate-stale-revision.test.ts test/integration/negotiation-guardrail.test.ts` → 4 files, 36/36 ✓ at real Postgres (`PgAuditRepository` read-back) → 4 · **§8.7 rung-5 lane (Fable gate on PR #1141, 2026-09-13 18:54Z):** `estimate-concurrent-approval-race-7-8.spec.ts` → 1 passed; both tenants race concurrently → **5 (T2)** |
| **7.9** | **As J**, I want a deposit before I order parts, so I'm not financing the customer | **Given** `before_approval` and an unpaid deposit, **when** the customer approves, **then** it is **blocked** and the estimate stays `sent` | **5** ↑ (T3) — `before_approval` blocks with the capped $99 shown and no Accept (approve → 409); the `after_approval` tenant accepts and the signed webhook credits the deposit; paying a live before-approval deposit stays parked (#1000/#1002) | `after_approval` proven. 🚨 **The `before_approval` block and the fixed-amount rule are in-memory only** · **G1 2026-09-12:** same file — `after_approval` at real Postgres, single tenant, audit in memory → 3; `before_approval` and the fixed-amount rule remain unit-only · **#1012 2026-09-12:** `before_approval` refuses acceptance until the deposit is paid, then allows (mapped `ConflictError`), and the fixed-amount rule caps the required deposit at the estimate total — both end-to-end through `PublicEstimateService.approve` at real Postgres; a second tenant with NO deposit rule accepts in the same run (T1·T3). No pricing/deposit defect found. `RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts test/integration/estimate-phases.test.ts test/integration/sweep-tenant-fanout.test.ts test/integration/estimate-stale-revision.test.ts test/integration/negotiation-guardrail.test.ts` → 4 files, 36/36 ✓ at real Postgres (`PgAuditRepository` read-back) → 4 · **§8.7 rung-5 lane (Fable gate on PR #1141, 2026-09-13 18:54Z):** `estimate-deposit-gate-7-9.spec.ts` → 2 passed (one an expected-fail pin): the rule set through the real `PUT /api/settings`, T3 across tenants, `deposit_paid_cents=5000` credited by a self-signed `checkout.session.completed`; the live deposit checkout with a placeholder key answers a raw 500 (`public-estimate-service.ts:883-886`, noted on #1110) → **5 (T3)** |
| **7.10** | **As M**, I want unviewed quotes chased automatically, so my pipeline doesn't die of silence | **Given** concurrent nudges for one estimate, **when** dispatched, **then** exactly one send, cadence advances once, audit rows written, a 48h cooldown respected, and a crash mid-send recovers | **4** ↑ (T1·T4) — reached except the automatic trigger: one send under concurrent nudges, cadence advances once, audits written, 48h cooldown, crash-mid-send recovery — via the production sweep function; the hourly `setInterval` (app.ts:6467) is pinned, not driven | **D:** `estimate-nudge.test.ts` — *nine real-DB tests* · **G1 2026-09-12:** `estimate-nudge.test.ts` **16** tests ✓ (not nine), audit assertions ×6 with two residual `InMemoryAuditRepository` refs, **T1** (`:682`, `:763`), **T4** via the `estimate-reminder sweep` fan-out entry. Rung 5 had no reachability run → 4 · **§8.7 rung-5 lane (Fable gate on PR #1141, 2026-09-13 18:54Z):** `estimate-nudge-sweep-7-10.spec.ts` → 2 passed (one an expected-fail pin) on the production `runEstimateReminderSweep` body against the webServer's Postgres; two concurrent sweeps → one send; the wall-clock trigger has no route/env → **4 (T1·T4)**; the minute-keyed dispatch idempotency is **#1145** |
...
| **8.5** | **As M**, I want ACH, cards on file and card-present all to settle correctly, including when they fail later | **Given** an ACH `processing → succeeded`, **then** one completed payment + paid invoice + audit chain; **given** `processing → payment_failed`, **then** the in-flight credit reverses and the invoice reopens; **given** a duplicate delivery, **then** no double-credit | **5** ↑ (T2, ACH) / **4−** (card on file, T1) reached / **3** (off-session) / **3** (card-present) | ACH is strong in all three directions. 🚨 **Card-present has no Docker-gated test; charging a stored card off-session has none at all** — storage round-trips, nothing proves money lands · **G1 2026-09-12:** ACH: `ach-webhook.test.ts` 4/4 ✓ with audit, **T1** (`:244`, cross-tenant/RLS) — 4 holds. Stored card: `customer-payment-methods.test.ts` 5/5 ✓ (**T1** `:77`, no audit event) proves storage at 4−; charging off-session is unit-only against a mocked `fetch` (`stripe-saved-card.test.ts`) → 3. Card-present: `stripe-terminal.test.ts` 10/10 ✓, mocked → 3 · **#1022 2026-09-12:** ACH re-run unchanged (`ach-webhook.test.ts` 4/4 ✓, audit chain via `findByCorrelation`, T1 `:244`) — 4 confirmed. **Card on file cannot take its audit read-back: saving a card emits no audit event anywhere** (`webhooks/routes.ts:1074-1139` logs with `logger.info` only; no `payment_method.*` event type, no `entityType: 'payment_method'` in `src`) — a money-code change, issue **#1057**; stays 4− (T1). Off-session (`stripe-saved-card.ts:184`, stubbed `fetch`) and card-present (`stripe-terminal.ts:236`, stubbed `fetch`; hardware) stay 3 — no Stripe test-mode key and no cassettes in the repo (the record/replay layer is LLM-only); parked on #1000 (PR #1055) · **§8.8 rung-5 lane (Fable gate on PR #1142, 2026-09-13 19:00Z):** `invoice-ach-card-on-file-8-5.spec.ts` → 2 passed: ACH `processing → succeeded` credits once with the audit chain, `→ payment_failed` reverses the in-flight credit and reopens, a duplicate delivery never double-credits (T2); the card-on-file half is reached the same way but its missing audit leg is product work → stays 4−; off-session / card-present need a Stripe test-mode key (#1000) → **ACH 5 (T2)** |
| **8.6** | **As M**, I want two payments arriving at once to both count, so my balance is never wrong | **Given** a $100 cash entry racing a $150 ACH webhook, **when** both commit, **then** both credit with no lost update; two concurrent full-balance payments credit **exactly once**; the SQL cap rejects a credit that no longer fits | **5** ↑ (T2) — a $100 cash entry racing a $150 ACH webhook both credit with no lost update; two concurrent full-balance payments credit exactly once; the SQL cap rejects | **D:** five Docker-gated files, nine tests — **the best-evidenced row in the money surface** · **G1 2026-09-12:** the concurrency cluster found by name is three files / seven tests (`payment-concurrent-credit` 1, `payment-duplicate-race` 3, `payment-reversal-concurrent` 3 — the "five files, nine tests" was not re-derived), all ✓, all single-tenant with no audit event asserted → 3 · **#1022 2026-09-12:** the three-file cluster now reads its audit leg back and meets a neighbour — `payment-concurrent-credit.test.ts`: both racing credits carry `PgAuditRepository`, one `payment.recorded` per credit matched to its `payments` row, a neighbour tenant races its own payment in the same `Promise.all` and never lands on this balance; `payment-duplicate-race.test.ts`: a replayed intent leaves exactly ONE `payment.recorded` and the SAME provider reference in a neighbour credits only that tenant (the partial unique index is `(tenant_id, reference_number)`); `payment-reversal-concurrent.test.ts`: `payment.reversed` read back beside the raced credit's row, and a neighbour can neither reverse this tenant's payment nor appear on its trail. `RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts test/integration/payment-concurrent-credit.test.ts payment-duplicate-race.test.ts payment-reversal-concurrent.test.ts` → 2/2 + 4/4 + 4/4 ✓. PROVEN-REAL-DB with audit, T1 → **4** (PR #1055) · **§8.8 rung-5 lane (Fable gate on PR #1142, 2026-09-13 19:00Z):** `invoice-concurrent-credit-8-6.spec.ts` → 2 passed, T2 → **5 (T2)** |
| **8.7** | **As M**, I want voiding an invoice to kill its payment link immediately, so nobody pays an invoice I cancelled | **Given** a void, **when** it commits, **then** `stripe_payment_link_id`/`_url` are NULL on the persisted row, in-flight intents are cancelled, and a `payment_link.deactivated` audit event is written | **5** ↑ (T2) — voiding nulls `stripe_payment_link_id/_url`, cancels in-flight intents and audits `payment_link.deactivated`; the link is dead on reload (hermetic no-key boot) | The *consequences* of void are proven at real DB (a voided invoice takes no credit). 🚨 **The deactivation itself is proven only against a mock provider — the exact scenario this story exists to prevent** · **G1 2026-09-12:** PROVEN-UNIT — invoice-payment-link, payment, invoice-payment-intent-cancel, P5-010D 67/67 ✓ (fake provider) · **#1022 2026-09-12:** D: `integration/invoice-void-payment-link.test.ts` (new) — the production `transitionInvoiceStatus` (`invoice.ts:583/599`) at real Postgres: `stripe_payment_link_id`/`_url` go NULL on the persisted row as `status` becomes `void`, `invoice.payment_link_deactivated` carries the link id + `reason: 'voided'`, `invoice.payment_intent_canceled` carries `pi_live_secret` only (the terminal `succeeded` intent is skipped), the `*_failed` events are asserted absent, a neighbour's live link survives and its void cannot be driven cross-tenant. `RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts test/integration/invoice-void-payment-link.test.ts` → 3/3 ✓. Persistence + audit legs PROVEN-REAL-DB, T1 → **4**; **the Stripe-side call is recorded against the in-repo `MockPaymentLinkProvider` (+ a test-local intent-capable fake), not proven** — the real deactivation needs a test-mode key (#1000) (PR #1055) · **§8.8 rung-5 lane (Fable gate on PR #1142, 2026-09-13 19:00Z):** `invoice-void-8-7.spec.ts` → 1 passed under the lane's documented env (no `STRIPE_SECRET_KEY`); with a placeholder key the mint calls real Stripe and answers a raw 500 (#1110 class) — env condition recorded, not a shortcut → **5 (T2)** |
| **8.8** | **As M**, I want a refund to adjust the record without lying about what happened, so my books stay honest | **Given** two concurrent deliveries of one `stripe_refund_id`, **when** processed, **then** `amount_refunded_cents` increments **exactly once**, one claim row exists, and the status is **never flipped** | **5** ↑ (T2) — one `stripe_refund_id` delivered twice increments `amount_refunded_cents` exactly once; one claim row; status never flipped | **D:** `payment-refunds.test.ts` — *interleave, stranded-claim, RLS and migration-backfill all covered* · **G1 2026-09-12:** `payment-refunds.test.ts` 5/5 ✓, **T1** (`:210` "refund claims are invisible to another tenant"), no audit event asserted → 4− · **#1022 2026-09-12:** `payment-refunds.test.ts` — the P0-4 interleave reads back exactly TWO `payment.refunded` rows for two applied refunds (`3000/3000`, `2000/5000`, correlated by refund id; the deduped redelivery returns before the audit write); the raw `payments` row keeps `amount_cents = 10000` and `status = completed` — only `refunded_amount_cents` moves; the neighbour's rejected attempt writes neither a claim row nor an audit row. `RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts test/integration/payment-refunds.test.ts` → 5/5 ✓. Audit leg closed: PROVEN-REAL-DB, T1 → **4** (PR #1055) · **§8.8 rung-5 lane (Fable gate on PR #1142, 2026-09-13 19:00Z):** `invoice-refund-8-8.spec.ts` → 1 passed, T2 → **5 (T2)** |
| **8.9** | **As M**, I want overdue invoices chased on a schedule **and never chased twice**, so I don't damage a relationship over a duplicate text | **Given** an invoice 15 days past due, **when** swept twice, **then** exactly **three** dunning rows exist with `step_key IN ('3:sms','7:sms','14:sms')` and a duplicate insert of `'7:sms'` raises 23505 | **5** ↑ (T2·T4) — the real leader-gated interval worker sweeps twice: exactly three dunning rows (`3:sms`,`7:sms`,`14:sms`), a duplicate `7:sms` raises 23505 | **Every cadence test uses an in-memory ledger.** The only real-DB dunning test keys on `manual:<proposalId>`. **Nothing proves the cadence key has ever met the real `UNIQUE` index** — and a duplicate sweep double-texting a customer about money is exactly the failure this story exists to prevent. **The largest single overclaim in the product** · **G1 2026-09-12:** `payment-reminder-dedup.test.ts` 5/5 ✓ (**T1** `:250`) keys on `manualReminderStepKey(proposalId)`; the cadence key (`3:sms`…) is tested only in memory (dunning-config, late-fee, estimate-nudge 37/37, with a hand-coded `23505`) — 3 stands, overclaim confirmed · **#1023 2026-09-12:** D: `integration/dunning-cadence.test.ts` (new) — an invoice **15 days past due, swept twice** (the criterion verbatim) on a 3/7/14-SMS cadence yields ledger rows `3:sms`/`7:sms`/`14:sms` in `invoice_dunning_events` at the real `UNIQUE (tenant_id, invoice_id, kind, step_key)`, three `send_payment_reminder` proposals and three `invoice.dunning_proposed` rows via `PgAuditRepository.findByEntity`; the duplicate `7:sms` — the key the criterion names — is a **raw INSERT on a pool client** refused with `23505` by the index itself (no repository, no hand-coded error); three sweeps leave two rows; a boundary pair pins the step's offset day in both directions (13 days past due → only `3:sms`/`7:sms`; exactly 14 → all three), guarding `elapsed < step.offsetDays` at dunning-schedule.ts:56; a second tenant on a 5-day-email cadence is chased on its own steps and cannot read the first's ledger. `RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts test/integration/dunning-cadence.test.ts` → 5/5 ✓ (RED first on every assertion). T4: `sweep-tenant-fanout.test.ts` gains an `overdue-invoice (dunning) sweep` entry (real enumerator, failure isolation, a not-yet-due neighbour untouched) → 27/27 ✓. PROVEN-REAL-DB, T1·T4 → **4**; the overclaim is closed. Rung 5 needs the owner-facing cadence control reached in a browser (PR #1053) · **§8.8 rung-5 lane (Fable gate on PR #1142, 2026-09-13 19:00Z):** `dunning-late-fee-8-9-8-10.spec.ts` (`PROCESS_ROLE=all OVERDUE_SWEEP_INTERVAL_MS=4000`) → 1 passed, T2 across the fan-out → **5 (T2·T4)** · **Row artifact at main (Fable, origin/main @ 859db9f94, 2026-09-15 19:21Z):** `e2e/journeys/dunning-late-fee-8-9-8-10.spec.ts` (`PROCESS_ROLE=all OVERDUE_SWEEP_INTERVAL_MS=4000`, api :38630, teardown TRUNCATE blocked) → 1 passed (34.3s); rows, per overdue tenant (91bda930, 971fc4cd): `invoice_dunning_events` `reminder` `3:sms`, `7:sms`, `14:sms` (exactly three each) + 3 `send_payment_reminder` proposals `ready_for_review` (`offsetDays` 3/7/14); audit `invoice.overdue` 1, `invoice.dunning_proposed` 4 per tenant. Evidence `regrade-evidence3/dunning-late-fee-8-9-8-10-1-rows.txt`; grade cell unchanged |
| **8.10** | **As M**, I want late fees applied consistently and capped, so I'm firm without being punitive | **Given** a re-executed `apply_late_fee`, **when** it runs again, **then** no second fee line appears on the real invoice; **and** a fee exceeding the cap is clamped | **4** (T4) — **story now met**: the owner sets a capped late fee in Settings → Late fees and the real overdue sweep drafts it once, clamped at the cap, and it lands on the invoice after approval. Rung 5 needs T3 (late-fee policy is per-tenant configuration); the proving spec's neighbour tenant is not overdue | **D:** `late-fee-idempotency.test.ts`, `voice-collections-execution.test.ts`. *The cap is unit-only* · **G1 2026-09-12:** `voice-collections-execution.test.ts` 3/3 ✓ with audit, **T1** (`:295`) → 4; `late-fee-idempotency.test.ts` 1/1 ✓ single tenant, no audit event → 3; the cap is unit-only; rung 5 had no reachability run · **#1023 2026-09-12:** `late-fee-idempotency.test.ts` extended — the re-execution now reads back exactly one `invoice.late_fee_applied` (`{stepKey:'initial', feeCents:2500, newAmountDueCents:17500}`); a second tenant takes its own fee (84000) while a foreign proposal is refused *not found in this tenant*; **the cap half is now at real DB through the real sweep** (`overdue-invoice-worker.ts:352` → `late-fee.ts:70-71`): flat 5000 with cap 2000 leaves one 2000 `Late fee` line and `amount_due_cents = 102000`, a re-sweep and a re-execution add nothing. `RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts test/integration/late-fee-idempotency.test.ts` → 3/3 ✓. Both halves PROVEN-REAL-DB, T1 → **4** whole; the voice-collections leg keeps G1's T1 (PR #1053) · **§8.8 rung-5 lane (Fable gate on PR #1142, 2026-09-13 19:00Z):** same spec proves the worker-level idempotency and clamp only when the test writes the config through the repository; `DunningConfigRepository.upsert` has no callers in `packages/api/src` → **#1143** · **Re-grade at main (Fable, origin/main @ 792f8d2b9, 2026-09-14 18:02Z):** `e2e/journeys/dunning-late-fee-8-9-8-10.spec.ts` (PROCESS_ROLE=all, OVERDUE_SWEEP_INTERVAL_MS=4000, real Postgres) → 1 passed: "the late fee the owner set in Settings is drafted once, clamped at the cap, and lands on the invoice after approval; a neighbour tenant is untouched" (the policy is saved through the Late fees sheet, `PUT /api/settings/dunning` 200); `sweep-tenant-fanout.test.ts` overdue-invoice sweep → 3/3 ✓ on the real enumerator (T4). #1143 fixed by #1176. No second overdue tenant with a different policy in the same run → **4 (T4)**, T3 is the gap to 5 · **Row artifact at main (Fable, origin/main @ 859db9f94, 2026-09-15 19:21Z):** `e2e/journeys/dunning-late-fee-8-9-8-10.spec.ts` (`PROCESS_ROLE=all OVERDUE_SWEEP_INTERVAL_MS=4000`, api :38630, teardown TRUNCATE blocked) → 1 passed (34.3s). The test title now includes #1193's T3 leg (a neighbour with its own policy), not re-graded here. Rows: `invoice_dunning_configs` 91bda930 `flat` value 5000 grace 5 max 2000 / 971fc4cd `percent` value 1000 grace 3 max 500; `invoice_dunning_events` `late_fee` `initial` 2000 / 500 (each at its own cap); `apply_late_fee` `executed` `feeCents` 2000 / 500; `invoice_line_items` "Late fee" 2000 (invoice total 40000 → 42000 after the fee) / 500 (9000 → 9500 after the fee); audit `invoice.late_fee_applied` 1, `proposal.approved` 1, `proposal.executed` 1 per tenant. Evidence `regrade-evidence3/dunning-late-fee-8-9-8-10-1-rows.txt`; grade cell unchanged |
| **8.11** | **As M**, I want to bill a big job in stages, so cash flow matches the work | **Given** a 3-milestone schedule on a total that doesn't divide evenly, **when** split, **then** Σ milestones === total, with the remainder milestone absorbing every stray cent | **4** ↑ (T1) — reached: a 3-milestone schedule on an uneven total splits with Σ === total and the remainder absorbing stray cents; 5 pending T2 and the spoken `create_invoice_schedule` seam (#1119-class) | **Unit only** · **G1 2026-09-12:** PROVEN-UNIT — 7 files 77/77 ✓ (`buildInvoiceSchedule` + in-memory repo) · **#1010 2026-09-12:** control lit — Milestone billing toggle (same spec as 8.3, 4 passed); copy corrected on review: milestones are drafted directly as numbered invoices, no approval step (`schedule-completion.ts`; 58121c872). Rung unchanged at 3 (unit) · **#1023 2026-09-12:** D: `integration/milestone-billing.test.ts` (new) — a 100.00 schedule split 3333/3333/remainder mints three persisted invoices at `milestone_index` 0/1/2 carrying 3333, 3333, **3334**; the sum read out of `invoices` equals `invoice_schedules.total_amount_cents`; the completion path (`schedule-completion.ts:79`) writes numbered drafts (`INV0001`…, never a `PENDING-` placeholder) with one labelled line, linked by `schedule_id`, one `invoice.milestone_minted` row each, idempotent on retry; with `milestoneBillingEnabled=false` only the deposit exists; a second tenant mints its own three and neither reads the other's schedule, invoices or audit rows. `RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts test/integration/milestone-billing.test.ts` → 4/4 ✓. PROVEN-REAL-DB, T1 → **4** (PR #1053) · **§8.8 rung-5 lane (Fable gate on PR #1142, 2026-09-13 19:00Z):** `milestone-billing-8-11.spec.ts` → 1 passed; no neighbour in the spec; no deterministic matcher for the spoken entry → **4 (T1)** reached |
...
| # | User story | Acceptance criterion | Rung | Confirm |
|---|---|---|---|---|
| **9.1** | **As M**, I want a customer thanked 2h after the job, so the last thing they remember is courtesy | **Given** two concurrent sweeps over one eligible job, **when** they run, **then** exactly one send and one `notification.thank_you_sms.sent` audit row — and a "sent" claim with a NULL stamp is **reconciled, not resent**; **and** the send survives the central consent gate in `block` mode | **4** (T4) — the real worker at real Postgres through the real consent gate: exactly one send and one audit row under two concurrent sweeps and on the reconcile path. Rung-5 reachability (the worker firing inside a running app) is not built; a failed audit write can still lose the audit (#1184) | **D:** `thank-you-sms-worker.test.ts` · **U:** `workers/thank-you-sms-worker.test.ts` — *the gated-chain case; see §12.2, this row was **unsendable in production** until 2026-09-12* · **G1 2026-09-12:** D 8/8 ✓ (audit ✓) + U 17/17 ✓; **T4** via the `thank-you-SMS sweep` fan-out entry — 4 stands · **#1013 2026-09-12 (grading):** the literal G4 grep on `thank-you-sms-worker.test.ts` is 0 by design (single-tenant unit file); the T4 evidence is `sweep-tenant-fanout.test.ts` → `thank-you-SMS sweep` (two real tenants, each SENT under its own scope, one throwing does not stop the other) — 4 (T4) stands · **§8.9 rung-5 lane (Fable gate on PR #1138, 2026-09-13 18:49Z):** `thank-you-sms-reachability-9-1.test.ts` drives the production worker with its real gates: exactly one send + one `notification.thank_you_sms.sent` under two concurrent sweeps, a "sent" claim with a NULL stamp reconciled not resent, consent `block` honoured; T2. The `it.fails` pin reproduces the crash-recovery reconcile path writing a second audit row for one send under a realistic interleaving → **#1140**; the "exactly one audit row" clause is not met on that path → stays **4 (T2)** · **Re-grade at main (Fable, origin/main @ 792f8d2b9, 2026-09-14 18:02Z):** `thank-you-sms-reachability-9-1.test.ts` → 3/3 ✓ incl. "FIXED (#1140): a second concurrent sweep tick never adds a second audit row" (fix #1160); `thank-you-sms-worker.test.ts` → 8/8 ✓; `sweep-tenant-fanout.test.ts` thank-you sweep → 2/2 ✓ on the real cross-tenant query (T4) → **4 (T4)** |
| **9.2** | **As M**, I want a review asked for automatically, so my rating grows without me thinking about it | **Given** a completed job 24h old, **when** swept twice, **then** one `feedback_send` enqueued, with the setting defaulting **on** at the column level | **4** ↑ (T1·T4) | **D:** `review-request-sweep.test.ts` — *no audit assertion* · **G1 2026-09-12:** `review-request-sweep.test.ts` 2/2 ✓, **T4** via fan-out; no audit event asserted (the file no longer uses `InMemoryAuditRepository` — the 4− now rests on the missing assertion) · **#1013 2026-09-12:** `review-request-sweep.test.ts` — the sweep's `feedback_send` is now driven through the REAL `createFeedbackSendWorker` with `PgCustomerRepository`/`PgSettingsRepository`/`PgFeedbackRequestRepository`/`PgDncRepository` and `GatedMessageDelivery` on `PgAuditRepository` (`enforcement: 'block'`): a no-consent customer yields `SmsSuppressedError('no_consent')` and the `sms.suppressed` row is read back via `findByEntity(tenantId, 'sms_message', customerId)` (the sweep itself never audits — the gate does, one hop downstream at `gated-message-delivery.ts`). `RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts test/integration/review-request-sweep.test.ts` → 3/3 ✓ (RED: expected `dnc`, got `no_consent`). T4 cited: `sweep-tenant-fanout.test.ts` review-request entry. Audit leg closed: PROVEN-REAL-DB, T1·T4 → **4** (PR #1070) · **§8.9 rung-5 lane (Fable gate on PR #1138, 2026-09-13 18:49Z):** `review-request-sweep-reachability-9-2.test.ts` confirms the production sweep at real Postgres: swept twice → one `feedback_send`, setting defaults on; T4 fan-out. No owner surface to reach → cell unchanged |
| **9.3** | **As M**, I want an unhappy customer routed to me privately and a happy one to Google, so a bad day doesn't become a permanent 2★ | **Given** `rating: 3`, **when** submitted, **then** the response persists and **no** review links return; **given** `rating: 5`, **then** the configured link returns | **5** ↑ (T2) — the public feedback page: rating 3 persists with no review links, rating 5 returns the configured link; neighbour untouched | Proven only by a mocked-repo route test. **This is the row deciding whether a 2★ experience becomes a public Google review, and it has no real-DB proof** · **G1 2026-09-12:** `feedback.test.ts` 5/5 ✓ (**T1** `:83`) but no rating-gate case; the unit route test enforces a "4★+" boundary, not the 3★-vs-5★ split this row words — 3 stands, and the row's wording should follow the code · **#1013 2026-09-12:** D: `integration/feedback-review-gating.test.ts` (new) — the real `POST /public/feedback/:token` (`routes/public-feedback.ts:137`, the `rating >= 4` boundary the code actually has): a 3★ response persists with **no** `reviewUrls` and its `feedback_response.submitted` row reads back; a 5★ returns the configured links; a second tenant with no review URLs gets none on a 5★ (the first tenant's URLs never leak). `RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts test/integration/feedback-review-gating.test.ts` → 3/3 ✓ + 1 expected fail. Criterion PROVEN-REAL-DB, T1 → **4**; 🚨🚨 closed. **Story half not in the criterion — *routed to me privately* — has no code path** (no push/SMS/email to the owner on a low rating; only the authenticated `GET /api/feedback`): pinned by the lane's `it.fails`, issue **#1071**. Harness note: `PgSettingsRepository.create()` does not insert the review-URL columns — set them via `update()` (PR #1070) · **§8.9 rung-5 lane (Fable gate on PR #1138, 2026-09-13 18:49Z):** `e2e/journeys/public-feedback-rating-gate-9-3.spec.ts` (real browser, real public route, dedicated api port) → 1 passed with a divergent neighbour tenant. → **5 (T2)** |
| **9.4** | **As M**, I want new Google reviews found and a response drafted for my approval, so I reply within a day without watching for them | **Given** a connected tenant, **when** a sweep runs, **then** new reviews persist, the cursor advances, a re-sweep persists nothing new, a 429 stamps backoff, and reviews are RLS-invisible cross-tenant — with a PII-redacted draft response awaiting approval | **4** ↑ (T4) sweep — audited per review and per backoff (#1136) / **5** ↑ (T2) — the PII-redacted draft response is approved by the owner in the real UI (#1138); classification/drafting still unit-only | **D:** `google-reviews-worker.test.ts`. *Classification and drafting are unit-only* · **G1 2026-09-12:** `google-reviews-worker.test.ts` 7/7 ✓, **T4** via fan-out, no audit event asserted → 4−; classification/drafting unit-only → 3 · **#1013 2026-09-12:** D: `integration/google-reviews-matching.test.ts` (new) — the pipeline's one DB-touching step, `PgCustomerLoader.findRecentCustomersWithName` (`reputation/match-customer.ts:181`, customers ⋈ jobs ⋈ appointments), at real Postgres: the 60-day recency window excludes a 90-day-old visitor, an identically named customer in a neighbour tenant never surfaces, `matchReviewerToCustomer` driven by the real loader resolves a confident match, and `buildReviewResponseProposal` keys the private follow-up + service credit to the REAL matched customer (only the two LLM draft calls are faked; `classifyReview` is pure). `RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts test/integration/google-reviews-matching.test.ts` → 5/5 ✓ (RED: expected 1, got 10000). Classification/matching half PROVEN-REAL-DB, T1 → **4**; the sweep half keeps G1's 4− (T4, no audit event); rung 5 needs a connected Google Business Profile (#1000) (PR #1070) · **Audit leg landed (Fable gate on PR #1136, 2026-09-13 16:15Z):** `workers/google-reviews.ts` emits `review.ingested` per persisted review and `review.sweep_backoff` on the 429 branch through the worker's audit repo (wired in `app.ts`); `google-reviews-worker.test.ts` asserts both across the fan-out (T4) and RLS invisibility (T1). The sweep half moves 4− → **4 (T4)**; the matching half stays 4 (T1); classification/drafting remain unit-only (the 3 noted earlier). RED (main's six product files restored, the branch's five integration files): 12 failed / 11 passed; GREEN: 23/23 at real Postgres, unit 2130/2130, tsc 0 errors introduced; the new event rows read back per tenant from the GREEN container. Emitters are best-effort (logged, never masking the real outcome) like their sibling emitters on the same mid-call / sweep / hook paths. · **§8.9 rung-5 lane (Fable gate on PR #1138, 2026-09-13 18:49Z):** `e2e/journeys/review-response-approval-9-4-9-5.spec.ts` → 1 passed: the drafted response awaiting approval is reviewed and approved on the real screen, rows read back, neighbour untouched (T2). The sweep half keeps its audit-leg dependency on #1136 (`review.ingested` / `review.sweep_backoff`). → approval half **5 (T2)** | |
| **9.5** | **As M**, I want to make a bad experience right without over-giving, so goodwill doesn't become a leak | **Given** $80 already issued in 12 months and a $50 tier proposed, **when** the cap applies, **then** the credit is **omitted, not zeroed** — because proposing "$0 credit" is worse than proposing none | **5** ↑ (T2) — with $80 issued in 12 months and a $50 tier proposed, the credit is omitted (not zeroed) on the real approval surface | 🚨 The existing test calls itself a *smoke test that stubs `pool.connect()`* · **G1 2026-09-12:** no integration file exists; unit 23/23 ✓ incl. `pg-service-credit.test.ts`, which stubs `pool.connect()` as its header says — 3 stands · **dormant-rows lane (Fable gate) 2026-09-12:** D: `integration/service-credit-cap-9-5.test.ts` (new; replaces the `pool.connect()` stub smoke test as the row's proof) — at real Postgres `PgServiceCreditRepository.sumIssuedInLast12Months` returns exactly $80 with $30 + $50 inside the window and $80 seeded 13 months outside it; `buildReviewResponseProposal` with a $50 tier OMITS the credit (`serviceCredit: null`, no `"amountCents":0` anywhere in the payload) and leaves the ledger byte-identical; a neighbour tenant's $80 is uncounted and unreadable; executing the capped proposal through the production registry issues nothing and `review_response.executed` reads back. Cap math (`applyCreditCap`, `CREDIT_CAP_CENTS_PER_12_MONTHS = 10000`) asserted, never changed. `RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts test/integration/service-credit-cap-9-5.test.ts` → 7/7 ✓ + 1 expected fail. Criterion PROVEN-REAL-DB, T1 → **4**. **Money defect surfaced:** the cap is enforced at draft time only — `executeServiceCredit` (`review-response-handler.ts:334-356`) inserts the payload amount with no re-read, so a $50 approved after another $50 landed executes to $140 against the $100 cap (the `it.fails`, visible in `service_credits`) → issue **#1080** (PR #1076) · **§8.9 rung-5 lane (Fable gate on PR #1138, 2026-09-13 18:49Z):** `review-response-approval-9-4-9-5.spec.ts` → the cap omits the service credit rather than proposing "$0"; T2. (#1080's execute-time cap question stays parked.) → **5 (T2)** |
| **9.6** | **As M**, I want one text at the end of the day telling me what happened, so I never open a dashboard | **Given** a tenant at its local digest time, **when** the sweep runs, **then** a digest sends once — not duplicated, not re-sent — carrying **"what I wasn't sure about"** and **"what I learned today"** | **5** ↑ (T2·T3·T4) — the digest toggle and `/digest` reached hermetically; the send is now audited (#1113) | The write **and both named sections** are proven at real Postgres. 🚨 **`digest_enabled` defaults false and *nothing in web or mobile writes it*.** `PUT /api/settings` does accept `digestEnabled` behind `settings:update`, so this is reachable by API — but Mike cannot turn the product's central promise on from any shipped surface · **G1 2026-09-12:** `daily-digest-worker.test.ts` 3/3 ✓ stubs compute deps empty and does **not** prove the two sections — that proof is `digest-reflection.test.ts` 5/5 ✓; **T3·T4** via the digest fan-out entry; no audit event asserted in either file → 4− · **#1010 2026-09-12:** client control lit — "Daily digest" toggle in Settings › Quick settings writes `PUT /api/settings {digestEnabled}`; `/digest` gains a supervisor-sidebar nav entry (`Shell-mode.test.tsx` 14/14 ✓). `e2e/journeys/digest-toggle.spec.ts` — `CLERK_DEV_HMAC_TOKENS=true E2E_USE_TEST_DB=true DB_SSL=false DATABASE_URL=<testcontainer> npx playwright test accept-invitation digest-toggle revenue-cluster-toggles --project=chromium` → 4 passed (48.1s), real Postgres, chromium project. Rung unchanged at 4− (no audit event asserted in the worker tests — §8.9 move ticket) · **9.6 lane (Fable gate) 2026-09-13:** D: `integration/daily-digest-send-9-6.test.ts` — the real `runDailyDigestSweep` at real Postgres: a tenant with the digest on and a completed job gets exactly one `daily_digests` row + one `message_dispatches` (sms, `sent`); a tenant with it off gets none; `digestChannel 'none'` is a documented skip (the row stores, nothing dispatches); T3 — two tenants in two timezones due at one instant each get their OWN send with their OWN counts (Phoenix 2 / Chicago 1); T4 cited — `sweep-tenant-fanout.test.ts` runs the PRODUCTION tenant selector over N seeded tenants on their own timezone + digest_time. Gate run: `RLS_RUNTIME_ROLE=true EXTERNAL_TEST_DB_URL=<kept container> npx vitest run --config vitest.integration.config.ts test/integration/daily-digest-send-9-6.test.ts test/integration/sweep-tenant-fanout.test.ts` → Test Files 2 passed (2) · Tests 31 passed | 1 expected fail (32); kept-container dump: 5 `message_dispatches` sms/sent, one per tenant, **0 digest audit rows**; planted control (T3 assertion flipped) → 1 failed. **Reachability (hermetic, #1004 shape):** `e2e/journeys/digest-toggle.spec.ts` — two tenants bootstrapped through the real Clerk-webhook + onboarding flow, the owner flips *Daily digest* in Settings, activity happens through the real jobs API (`new → scheduled → in_progress → completed`), the sweep runs as the product's own worker tick (`runDailyDigestSweep` against the same DATABASE_URL — there is no route that fires it), and `/digest` shows *Jobs completed 1* — never the neighbour's 2 (T2). `CLERK_DEV_HMAC_TOKENS=true DB_SSL=false DATABASE_URL=<testcontainer> E2E_USE_TEST_DB=true VITE_CLERK_PUBLISHABLE_KEY=… npx playwright test e2e/journeys/digest-toggle.spec.ts --project=chromium --retries=0` → 2 passed (1.1m) (real Postgres, chromium project, screenshots `close-9-6/01-digest-enabled.png`, `02-digest-content-reached.png`). **Why not 4 or 5:** nothing in the digest send path writes an audit event — `DailyDigestWorkerDeps` has no `auditRepo`; the one in `DigestComputeDeps` is read-only (pinned after review as a direct zero-audit-rows assertion in `daily-digest-send-9-6.test.ts`, which the **#1113** fix must flip to the positive read-back). PROVEN-REAL-DB write without its audit leg = REAL-DB-WRITE-ONLY → **4−**, now T2·T3·T4 and reached; flips to 5 the day #1113 lands and the pin turns green · **Audit leg landed (Fable gate on PR #1136, 2026-09-13 16:15Z):** `workers/daily-digest-worker.ts` emits `notification.daily_digest.sent` / `.suppressed` (with reason `channel_none` / `no_owner_phone` / `no_sms_transport`) / `.failed` through the worker's audit repo (wired in `app.ts`); `daily-digest-send-9-6.test.ts` asserts the sent and suppressed rows across three tenants in one run (T2·T3·T4). With the hermetic reachability from PR #1111 / batch #1116 (`digest-toggle.spec.ts`: the owner lights the toggle, the digest renders on `/digest`), write + audit + reachability → **5 (T2·T3·T4)**. #1113 closes on merge; the suppressed-is-audited choice is Josh's to reverse there. RED (main's six product files restored, the branch's five integration files): 12 failed / 11 passed; GREEN: 23/23 at real Postgres, unit 2130/2130, tsc 0 errors introduced; the new event rows read back per tenant from the GREEN container. Emitters are best-effort (logged, never masking the real outcome) like their sibling emitters on the same mid-call / sweep / hook paths. |
| **9.7** | **As S**, I want a weekly summary I can read Saturday morning, including how often it repeated a mistake, so I can see whether it's learning | **Given** a week of corrections, **when** summarised, **then** total / repeats / rate come from the real corrections table, and the field is **omitted at zero**; the send ledger is idempotent and a failed send leaves **no** row so the week retries | **4** ↑ (T3·T4) | **D:** `weekly-feedback-builder.test.ts`, `hfcr-weekly-send-worker.test.ts`, `sweep-tenant-fanout.test.ts` (per-tenant recipient / greeting / opt-out, through the production resolvers) · **G1 2026-09-12:** `weekly-feedback-builder.test.ts` 5/5 + `hfcr-weekly-send-worker.test.ts` 6/6 + `sweep-tenant-fanout.test.ts` 16/16 ✓ (production resolvers, **T3·T4**); no audit event asserted → 4− · **#1011 2026-09-12:** owner-facing reachability lit for the row's "cannot turn it off" gap. `e2e/journeys/weekly-feedback-toggle.spec.ts`: `DB_SSL=false DATABASE_URL=<testcontainer> E2E_USE_TEST_DB=true VITE_CLERK_PUBLISHABLE_KEY=pk_test_ZHVtbXkuY2xlcmsuYWNjb3VudHMuZGV2JA== npx playwright test weekly-feedback-toggle.spec.ts --project=chromium --reporter=line` → 1 passed (real Postgres testcontainer, chromium). Owner flips "Weekly summary email" OFF in Settings › Quick settings; `PUT /api/settings {weeklyFeedbackEnabled:false}` persists, read back via `GET /api/settings` after a full page reload; `tenant_settings.weekly_feedback_enabled` row confirmed `f` with a `settings.tenant.updated` audit row. Evidence class D, T1 (single tenant here; cross-tenant scoping for this worker is already T3 via `sweep-tenant-fanout.test.ts`'s weekly-feedback entry). Rung unchanged (Fable states rungs) · **#1011 PR-3 gate (Fable) 2026-09-12:** re-run locally — `weekly-feedback-toggle.spec.ts` 1 passed at a real Postgres testcontainer, `tenant_settings.weekly_feedback_enabled = f` and its `settings.tenant.updated` row polled during the run. The *cannot turn it off* gap that held this row at 4− is closed by an owner-facing control reached in a browser → **4** (T3·T4 from the sweep entry; the browser leg itself is T1). Rung 5 needs the email leg captured hermetically (a sink for the Saturday send), not just the switch (PR #1062) · **§8.9 rung-5 lane (Fable gate on PR #1138, 2026-09-13 18:49Z):** `weekly-feedback-reachability-9-7.test.ts` confirms the production weekly summary at real Postgres (totals/repeats/rate from the real corrections table, field omitted at zero, ledger idempotent, failed send leaves no row); T3·T4. No owner surface to reach → cell unchanged |
| **9.8** | **As M**, I want a correction I make once to stick, so I never fix the same thing twice | **Given** a labor-rate correction, **when** executed, **then** the config changes so the **next same-day draft reflects it**, it appears in the day's applied lessons, and `correction_lesson.applied` is written | **4** ↑ (T2) — **story now met on the real pipeline**: an owner correction through edit→approve→execute records a `correction_lesson` from the AI draft vs the executed payload and cascades it; a neighbour's divergent correction yields its own lesson. Rung-5 reachability (the owner's edit in the app) is not proven | **D:** `correction-loop.test.ts` · **G1 2026-09-12:** `correction-loop.test.ts` 4/4 ✓ with audit, **T1** (`:145` FORCE RLS across tenants); rung 5 had no reachability run → 4 · **#1013 2026-09-12 (grading):** `correction-loop.test.ts:145` `FORCE RLS isolates correction_lessons across tenants` — a real second tenant, `findById`/`findAppliedForDay` empty from the other side: T1 confirmed, 4 stands · **§8.9 rung-5 lane (Fable gate on PR #1138, 2026-09-13 18:49Z):** `correction-loop-reachability-9-8-9-9.test.ts` drives a real owner correction through the real approve→execute pipeline: no `correction_lesson` is ever recorded because `ProposalExecutor` mirrors `proposal.payload` into `executedPayload` (`executor.ts:279/427`), so the diff in `record-on-execution.ts:114-126` is always empty (`it.fails` pin, reproduced at real Postgres) → **#1139** · **Re-grade at main (Fable, origin/main @ 792f8d2b9, 2026-09-14 18:02Z):** `correction-loop-reachability-9-8-9-9.test.ts` → 3/3 ✓, no `it.fails` left (fix #1161 for #1139): POSITIVE (neighbour untouched, T2) and 9.8 (tenant B's divergent correction produces its own lesson and rate); `executor-executed-payload-money-guard-1139.test.ts` → 2/2 ✓ (`executed_payload` stays the executed money payload) → **4 (T2)** |
| **9.9** | **As M**, I want to undo a lesson it learned wrong, so teaching it is not a one-way door | **Given** an applied lesson, **when** undone, **then** the prior value is restored **exactly**, `correction_lesson.reverted` is emitted **exactly once**, a second undo is a no-op, and it drops from the day | **4** (T2) — **story now met on the real pipeline**: the owner reverses a lesson a real executed correction recorded, and the labor rate it cascaded, through undo's explicit lessons scope; the neighbour tenant's lesson stays. Rung-5 reachability (undo in the app) is not proven | **D:** `correction-loop.test.ts` — *asserts both audit ends with `PgAuditRepository`; the best-evidenced row in the lifecycle sections* · **G1 2026-09-12:** same file — but "a second undo is a no-op" has **no test**; the idempotent-revert clause is unproven → 4 with that gap · **#1013 2026-09-12 (grading):** same file/assertion as 9.8 — T1 confirmed, 4 stands; the *second undo is a no-op* clause still has no test · **second-tenant lane (Fable gate) 2026-09-12:** the missing clause is proven — after a first undo (price back to 9000) and an operator edit to 9999, a SECOND undo of the same lesson is a no-op (`undoCorrectionLesson`'s `status === 'reverted'` short-circuit): price stays 9999 and exactly one `correction_lesson.reverted` row exists. `RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts test/integration/correction-loop.test.ts` → 5/5 ✓ (RED: expected 9000, got 9999). Criterion now whole at 4 (T1) (PR #1074) · **§8.9 rung-5 lane (Fable gate on PR #1138, 2026-09-13 18:49Z):** same file: even granting a lesson existed, `undoProposal` (`actions.ts:512-515`) refuses any proposal not in `approved`, and a lesson can only exist after execution → the reversal path is unreachable through the product (`it.fails` pin) → **#1139** · **Re-grade at main (Fable, origin/main @ 792f8d2b9, 2026-09-14 18:02Z):** `correction-loop-reachability-9-8-9-9.test.ts` → 3/3 ✓ incl. "9.9: the owner reverses the lesson a real executed correction recorded (and the labor rate it cascaded) through undoProposal's explicit lessons scope" with tenant B driven through the same pipeline (fix #1161) → **4 (T2)** |
| **9.10** | **As M**, I want a mistake I've corrected three times to become a permanent fix I approve, so the system stops needing me for it | **Given** a third same-target correction, **when** it lands, **then** a meta-proposal is minted that, once approved, updates the real catalog **through the production registry and executor** | **4** ↑ (T1) — reached: the third same-target correction mints the meta-proposal and its approval updates the real catalog through the production registry + executor in the real browser; 5 pending a T2 leg | **D:** `correction-repetition-meta-proposal.test.ts` · **G1 2026-09-12:** `correction-repetition-meta-proposal.test.ts` 2/2 ✓ with audit, single tenant → 3 · **#1013 2026-09-12 (grading):** `createTestTenant` is called once in `correction-repetition-meta-proposal.test.ts` (line 72) and no cross-tenant assertion exists — single tenant, T0 confirmed, 3 stands · **second-tenant lane (Fable gate) 2026-09-12:** `correction-repetition-meta-proposal.test.ts` — tenant B's two same-target corrections mint no meta-proposal while tenant A's third mints exactly one `update_catalog_item`; tenant B cannot read it (`findByCorrectionTarget`, `findById` null) and `findByEntity` under B is empty; `correction_repetition.proposed` read back under A. `RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts test/integration/correction-repetition-meta-proposal.test.ts` → 3/3 ✓ (RED: expected 1 proposal for B, got 0). T1 → **4** (PR #1074) · **§8.9 rung-5 lane (Fable gate on PR #1138, 2026-09-13 18:49Z):** `e2e/journeys/correction-meta-proposal-approval-9-10.spec.ts` → 1 passed (owner approves in the real inbox; catalog row updated; `proposal.approved/executed` audited). No neighbour in the spec → **4 (T1)** reached |
...
| **9.12** | **As M**, I want one inbox for every channel with a reply drafted for me, and **nothing sent without my hand on it** | **Given** a thread, **when** I ask for a suggestion, **then** a draft returns and **zero** dispatch rows are written; **given** a DNC number, **then** a reply writes **no** dispatch row at all | **4** ↑ (T1) guarded send — the DNC refusal and a failed dispatch are audited (#1136) / **4** ↑ (T1) — the AI-suggestion leg is reached in the real inbox: a draft returns and zero dispatch rows are written (#1138); 5 pending T2 | Inbox and guarded send proven — including the DNC refusal, which is the "never auto-sent" half. 🚨 **The AI-suggestion leg has no integration test at all** · **G1 2026-09-12:** `conversation-inbox.test.ts` 2/2 ✓ (**T1** `:182`) + `conversation-reply-send.test.ts` 2/2 ✓ (incl. the DNC refusal); no audit event asserted → 4−; the AI-suggestion leg is unit-only (51/51) → 3 · **#1013 2026-09-12:** `conversation-inbox.test.ts` — the file's T1 was vacuous (a neighbour with nothing seeded, `.every` over an empty list); replaced by a real fixture: a neighbour tenant's unanswered thread surfaces in its own listing and never in the first tenant's. `RLS_RUNTIME_ROLE=true npx vitest run --config vitest.integration.config.ts test/integration/conversation-inbox.test.ts` → 2/2 ✓ (RED: flipped to `.toContain`). T1 is now real; the cell holds — the reply-send audit leg (`conversation-reply-send.test.ts`) was not touched (4− stands) and the AI-suggestion leg (`POST /api/conversations/:id/suggest-reply`, `SuggestReplyTask`, on demand — not a stored draft) stays unit-only (3). The lane's expected-fail test asserting a stored `replyDraft` field was removed at the gate as mis-specified (PR #1070) · **Audit leg landed (Fable gate on PR #1136, 2026-09-13 16:15Z):** `conversations/reply-service.ts` emits `conversation.reply.suppressed` (DNC block — previously no row of any kind) and `conversation.reply.failed` (provider throw) through the service's wired audit repo; `conversation-reply-send.test.ts` asserts both at real Postgres, T1. The guarded-send half moves 4− → **4 (T1)**; the AI-suggestion leg is unchanged (unit-only → 3). RED (main's six product files restored, the branch's five integration files): 12 failed / 11 passed; GREEN: 23/23 at real Postgres, unit 2130/2130, tsc 0 errors introduced; the new event rows read back per tenant from the GREEN container. Emitters are best-effort (logged, never masking the real outcome) like their sibling emitters on the same mid-call / sweep / hook paths. · **§8.9 rung-5 lane (Fable gate on PR #1138, 2026-09-13 18:49Z):** `e2e/journeys/conversation-suggest-reply-9-12.spec.ts` → 1 passed (real thread, real suggest route, `message_dispatches` count unchanged). The DNC half keeps its audit-leg dependency on #1136 (`conversation.reply.suppressed`). No neighbour in the spec → suggestion half **4 (T1)** reached (was unit-only, 3) | |
