...
- **The most dangerous voice defect:** voice-dictated job notes (`add_note`) pass the approval gate and then **fail at execution** (`targetId` never populated from the resolved entity) — the owner approves, the note never saves, and while the inbox does surface the failure after the fact (`execution_failed` + `executionError` rendered in `InboxPage.tsx:906`), the dictated content is recoverable only by a screen visit and retry (B7.4).
...
| B1.2 | tenant_id + RLS FORCE everywhere | ⚙️ | 4 | Real-DB proof: `test/integration/rls-force-catalog.test.ts` (applied-catalog FORCE-flag introspection) + `test/integration/rls-tenant-isolation.test.ts` and `rls-cross-tenant-sweep.test.ts` (behavioral cross-tenant negatives) `[suite]`; fast text-level guard `test/db/schema.test.ts` ran 17/17 (119/119, exemptions pinned `schema.ts:5315-5331`) | Runtime role enforcement is boot-asserted (`config.ts:399-411`) but rung 6 unverified. (Round-7 correction: the schema.test.ts citation alone was text-parsing, not real-DB — the catalog/behavioral integration tests are the rung-4 proof) |
...
| Req | Requirement | Tag | Rung | Evidence | Missing link |
|---|---|---|---|---|---|
| B3.1 | AI answers 24/7 in brand voice | ⚙️ | 3 | `routes/telephony.ts:337-481` live entry; after-hours :421-444; brand greeting `twilio-adapter.ts:457-498` | Locked brand voice reaches only the greeting (persona resolver loads agent name + greeting) and negotiation TTS (`create-voice-turn-processor.ts:1863-1864`) — general call dialogue does not consume the six-field brandVoice config. Sweep #21: rung-4 pair (audit assertion + cross-tenant negative) not evidenced in the cited real-DB tests — capped under R1. |
| B3.2 | Caller resolved; unknowns → lead | ⚙️ | 3 | `identify-caller.ts:18-77` @ `twilio-adapter.ts:1216`; leads :1329; `identify-caller.test.ts` `[suite]` covers phone matching only | The unknown-caller → lead-creation mutation has no real-DB test with audit + cross-tenant assertions — rung 4 unmet under R1 (round 5; row edit re-applied in round 9 after a silent patch failure) |
...
| B3.5 | Vulnerability signals elevate urgency | ⚙️ | 3 | `vulnerability-grader.ts` + hook wired both transports (`app.ts:4266-4267,4362-4363,4492-4493`); RLS proof `tenant-isolation.leak.test.ts:748-789` `[suite]` | `weather` signal is LLM self-report — the weather-provider integration was cut (`vulnerability-triage-hook.ts:134-139`) Sweep #21: rung-4 pair (audit assertion + cross-tenant negative) not evidenced in the cited real-DB tests — capped under R1. |
...
| B3.10 | Unclaimed inbound SMS → threaded convo | ⚙️ | 3 | Terminal fallback slot (`sms/inbound-dispatch.ts:114-124`) @ `app.ts:3461-3462`; RLS proof `inbound-sms-capture.test.ts:207-227` `[suite]` | Persistence + cross-tenant proven but the audit-event assertion is unexercised — rung 4's audit leg unmet under R1 (round 5; row edit re-applied in round 9 after a silent patch failure) |
...
| B4.9 | New customer + job = one combined proposal | ⚙️ | 3 | Atomic single `create_booking` (`public-booking.ts:333-439`); voice: one proposal + SCH-02 job auto-open (`handlers.ts:386-459`) | Sweep #21: rung-4 pair (audit assertion + cross-tenant negative) not evidenced in the cited real-DB tests — capped under R1. |
...
| B8.8 | Customer approves via token link | ⚙️ | 3 | `public-estimates.ts` unauthenticated (`app.ts:3024`); token ≥16 (:526); `/e/:id` public (`routes.ts:45`) | Round 8: the token-approval mutation's audit assertions live in in-memory-repo tests (`estimate-phases.test.ts` injects InMemoryAuditRepository; `audit-coverage-d2-1d` fully in-memory) and no cross-tenant negative exists — rung-4 pair unmet; the sweep's route carve-out was misapplied to a mutation row. |
...
| B8.13 | Deposit on acceptance, both policies | ⚙️ | 3 | Both policies (`public-estimate-service.ts:346-628`); Stripe link :736; webhook credit (`webhooks/routes.ts:1167-1203`); pay surfaces everywhere money is owed; `deposit-concurrent-credit.test.ts` `[suite]` | Sweep #21: rung-4 pair (audit assertion + cross-tenant negative) not evidenced in the cited real-DB tests — capped under R1. |
...
| Req | Requirement | Tag | Rung | Evidence | Missing link |
|---|---|---|---|---|---|
| B9.1 | Invoice from a spoken sentence | 🎙️ | 3 | Full chain; resolver-authored customer id (`invoice-task.ts:254-261`); `draft-invoice-execution.test.ts:156-194` cents+audit+cross-tenant `[suite]`; issue leg also voice-reachable (`issue-invoice-conversation-resolution.test.ts` `[suite]`) | One utterance + one tap yields a **draft**, not a payable invoice — issuing requires a second utterance + tap. Whether B9.1's "issued from a spoken sentence" is satisfied by that two-step flow is a **Part F interpretation decision**, not a scoring call; additionally the issue leg's integration test carries audit assertions but **no cross-tenant negative**. Capped at 3 pending both (round 7) |
| B9.2 | Estimate → invoice w/ review step | 📱 | 3 | `convert-estimate.ts:45-150` idempotent, draft status, audit; UI sheet | Sweep #21: rung-4 pair (audit assertion + cross-tenant negative) not evidenced in the cited real-DB tests — capped under R1. |
| B9.3 | Auto-invoice on completion | ⚙️ | 3 | Opt-in proposal-raising (`auto-invoice-on-completion.ts:55-96`); fires from route AND voice handler (`completion-effects.ts:47-63`) | Sweep #21: rung-4 pair (audit assertion + cross-tenant negative) not evidenced in the cited real-DB tests — capped under R1. |
...
| B9.8 | Partial payments + deposits | ⚙️ | 3 | Status machine (`invoices/payment.ts:43-80`); per-partial receipts; both deposit policies; `deposit-concurrent-credit.test.ts` `[suite]` | Round 7: `deposit-concurrent-credit.test.ts` asserts neither audit event nor cross-tenant negative, and the partial-payment receipt path is unexercised — rung-4 pair unmet. |
| B9.9 | Saved cards / off-session auto-pay | ⚙️ | 3 | SetupIntent off_session (`stripe-saved-card.ts:108` → `public-portal.ts:1015`); `chargeOffSession` → dues collector → 60s worker (`app.ts:5708-5760`) | Sweep #21: rung-4 pair (audit assertion + cross-tenant negative) not evidenced in the cited real-DB tests — capped under R1. |
...
| B9.11 | Late fees | ⚙️ | 3 | Sweep-raised proposals; UUIDv5-idempotent line, integer cents (`apply-late-fee-handler.ts`); `late-fee-idempotency.test.ts` `[suite]` | Sweep #21: rung-4 pair (audit assertion + cross-tenant negative) not evidenced in the cited real-DB tests — capped under R1. |
...
| B9.13 | Memberships: auto-renew, member pricing, priority booking | ⚙️ | 3 | All three wired: `agreement-run.ts` per sweep tick; `member-pricing.ts:29-54` @ estimates:232/invoices:174; horizon widening `public-portal.ts:583,670`; `agreements.test.ts` `[suite]` | Sweep #21: rung-4 pair (audit assertion + cross-tenant negative) not evidenced in the cited real-DB tests — capped under R1. |
| B9.14 | Receipt on payment | ⚙️ | 3 | Every successful `recordPayment` incl. recovery branch (`invoices/payment.ts:96-190`); per-partial receipts | Sweep #21: rung-4 pair (audit assertion + cross-tenant negative) not evidenced in the cited real-DB tests — capped under R1. |
...
| Req | Requirement | Tag | Rung | Evidence | Missing link |
|---|---|---|---|---|---|
| B10.1 | Automated review request | ⚙️ | 3 | 24h sweep, default TRUE (`review-request-worker.ts:60-80`, `schema.ts:5236`) @ `app.ts:6547`; `review-request-sweep.test.ts` `[suite]` | Sweep #21: rung-4 pair (audit assertion + cross-tenant negative) not evidenced in the cited real-DB tests — capped under R1. |
...
| B10.10 | Owner replies send directly, audited+DNC; AI via proposals | 📱 | 3 | `reply-service.ts:347` direct+DNC+audit; `conversation-reply-send.test.ts` cross-tenant+DNC `[suite]`; comms class never auto-approves | Sweep #21: rung-4 pair (audit assertion + cross-tenant negative) not evidenced in the cited real-DB tests — capped under R1. |
...
| B3 Capture | 13 | 1 | – | 2 | 6 | – | 4 | 3.23 |
| B4 Book | 10 | – | – | 1 | 6 | – | 3 | 3.50 |
| B5 Dispatch | 9 | 1 | – | 3 | 3 | – | 2 | 2.78 |
| B6 Execute | 4 | – | – | – | 3 | – | 1 | 3.50 |
| B7 Narrate | 12 | 1 | – | – | 7 | – | 4 | 3.42 |
| B8 Quote | 14 | – | 1 | – | 8 | – | 5 | 3.57 |
| B9 Bill | 15 | – | – | – | 12 | 1 | 2 | 3.33 |
| B10 Close | 10 | – | – | – | 6 | – | 4 | 3.80 |
| **Total** | **117** | **7** | **1** | **14** | **63** | **2** | **30** | **3.21** |
...
After the strict sweep, the strongest sections are B10 Close (3.80) and B8 Quote (3.57); the B9 money loop's wiring is broad but its real-DB proof inventory is thinner than first credited (mean 3.33). Its rung-6 story remains empty (no live payment ever attempted, no tenant bank connected today).
...
Of 19 🎙️ requirements: **2 reach rung 5** (B7.1, B7.7) → **11% strict voice coverage**. **8 are functionally completable by voice** (adds B4.7, B6.3, B7.6, B8.1, B9.1, B9.4) → 42%. The break points:
...
| B9.1 invoice | ✓ completable (rung 3) | Two-step draft→issue flow (Part F interpretation) + issue-leg test lacks cross-tenant |
...
| PostHog | UNKNOWN — ingestion observed, attribution unavailable | UNKNOWN — same | N/A | Probe: Rivet events ARE ingesting through 2026-07-28 (`proposal_executed` 238/30d, `job_created` 527, …) — but the taxonomy has **no environment property**, so the events cannot be attributed to production vs staging vs a local process, nor to a specific service. Also: no `$pageview` events — NOT evidence of absent web capture (the web client loads analytics lazily behind `VITE_POSTHOG_KEY` and does not necessarily emit pageviews; browser liveness is UNKNOWN); **no `$ai_generation`** (PostHog LLM analytics not wired) |
...
2. **Dictated job notes approve and then fail.** `add_note` voice payload carries only `targetReference`; `missingFields` stays empty; execution requires a UUID and fails after the owner's approval (`voice-extended-tasks.ts:460-484` vs `voice-extended-handlers.ts:94-99`). Correction (round 10): the failure IS surfaced — `execution_failed` with its `executionError` renders in the inbox (`InboxPage.tsx:260-267,906`) — so this is approve-then-visible-failure, not silent loss; but the note never saves, approval was already granted, and recovery needs a screen visit. Still the Gap-2 class on the highest-frequency narrate action, one notch less dangerous than first stated.
...
21. **Exhaustive strict sweep (2026-07-29) — closing the review loop deterministically.** Rather than absorbing further single-row review findings, every remaining rung-5 row with a mutation/outbound-comm substance was checked against its cited integration test for the full rung-4 pair (audit assertion + cross-tenant negative), by direct inspection of the test files. Twelve rows failed and were capped at 3: B3.1 (brand voice reaches only greeting + negotiation TTS — `create-voice-turn-processor.ts:1863` is the sole general-dialogue consumer), B3.5, B4.9, B8.13 (`deposit-concurrent-credit.test.ts`: zero audit/cross-tenant assertions), B9.2 (`estimate-phases`: no cross-tenant), B9.3, B9.9 + B9.13 (`agreements.test.ts`: zero audit assertions), B9.11 (`late-fee-idempotency`: no cross-tenant), B9.14, B10.1 (`review-request-sweep`: zero audit), B10.10 (`conversation-reply-send`: zero audit assertions). B9.10 was retained at 5 — `payment-reminder-dedup.test.ts` carries both legs, contradicting an earlier Track B note. Rows kept at 5 without the pair are those whose substance is a read-only surface, pure UI, structural property, or self-proving contract test (R1 carve-out), each judged individually. None of the twelve is 🎙️-tagged — voice coverage is unchanged (3/19 strict, 8/19 functional). New distribution: 0:7 / 1:1 / 2:14 / 3:60 / 4:3 / 5:32, mean 3.26. This sweep supersedes piecemeal correction rounds; the fix run's re-measurement remains the definitive arbiter.
22. **Seventh review round (2026-07-29).** Four dispositions: **B9.1 5→3** — the two-step draft→issue reading was a logged judgment call, but the honest surface is that one utterance + one tap yields a draft, not a payable invoice; the interpretation ("does B9.1 mean single-utterance-to-issued?") is routed to Part F as a product decision rather than resolved by scoring, and the issue leg's test additionally lacks a cross-tenant negative. **B9.8 4→3** (`deposit-concurrent-credit.test.ts` carries neither rung-4 leg — consistent with the #21 standard that capped B8.13 on the same file). **B1.2 retained at 4 with corrected citations** — the reviewer was right that `schema.test.ts` is migration-text parsing, but `rls-force-catalog.test.ts` (applied-catalog introspection) + `rls-tenant-isolation`/`rls-cross-tenant-sweep` in the passed suite supply the genuine real-DB proof. **PostHog browser-capture claim withdrawn** — the web client loads analytics lazily behind `VITE_POSTHOG_KEY` and does not necessarily emit `$pageview`; the event's absence was never a valid liveness probe; browser liveness is UNKNOWN. Final: distribution 0:7/1:1/2:14/3:62/4:2/5:31, mean 3.23; strict voice **2/19 (11%)**, functional 8/19. Phase 1 item 10 extended to restore B9.1's proof leg; the single-utterance-to-issued question is a Part F entry.
23. **Eighth review round (2026-07-29).** **B8.8 5→3** — the sweep's route carve-out was misapplied: customer token approval is a mutation, and its audit assertions exist only in in-memory-repo tests with no cross-tenant negative anywhere. Stale narrative means corrected (B9 3.53→3.33, B8 3.71→3.57 in the strongest-sections sentence). Final: distribution 0:7/1:1/2:14/3:63/4:2/5:30, mean 3.21; voice unchanged (2/19 strict, 8/19 functional).
24. **Ninth review round (2026-07-29).** (a) **Row/ledger desync fixed**: the round-5 downgrades of B3.2 and B3.10 updated the rollups but the row edits silently failed to apply (string-match no-op) — the rows are now corrected to rung 3, restoring row↔rollup reproducibility at 0:7/1:1/2:14/3:63/4:2/5:30, mean 3.21. (b) **Pre-award removed from the Phase 1 prompt**: B9.1 and B1.18 were counted toward the 14/19 target on the strength of Part F entries not yet decided. The prompt now states the target conditionally — 14/19 only if the product owner ratifies the recommended readings (B9.1: two-step issuance; B1.18: lock-by-tap amendment); if either decision goes the other way, that item stays red until the implied build ships, and the re-measurement reports the actual number (12–14/19), never the assumed one.
25. **Tenth review round (2026-07-29, applied post-merge in a follow-up PR).** (a) The Phase 1 DoD checkbox still demanded an unconditional 14/19 after the mission was made conditional — fixed (12–14/19 per ratified Part F decisions — B1.18 and B9.1 are independent contingencies; a goal-runner must never pre-award to satisfy a checkbox, and must never be unable to finish because a product decision went the other way). (b) The add_note headline softened from "no error reaches them" to approve-then-visible-failure — verified `execution_failed` + `executionError` render in the inbox (`InboxPage.tsx:260-267,906`); the note still never saves and the finding keeps its punch-list rank. (c) `track-c-probes.md`'s Stripe section synced to the current-account-scoped wording adopted in round 7.
...
- ✅ Voice coverage computed and stated (11% strict / 42% functional after post-review corrections — run log #16–22), with the definitional split disclosed.
