...
- **Weakest lifecycle section: B2 Situational Context** (mean rung 1.4) — geocoding does not exist anywhere in the shipping code, there is no place/address entity kind, the four-level address-bias order is absent, and service territory is consulted only by two structured web forms, never by voice.
...
| B7.6 | Spoken line-item to existing estimate | 🎙️ | 5 | `update_estimate` chain; `resolveEstimateIdGate` lifts on unambiguous match (`estimate-edit-task.ts:265,350-356`); required-deps handler (`handlers.ts:1400`); 31/31 ran | — |
...
| Req | Requirement | Tag | Rung | Evidence | Missing link |
|---|---|---|---|---|---|
| B8.1 | Estimate from spoken description or photo | 🎙️ | 5 | Voice chain contract-pinned (ran 5/5); resolver-authored gate (`estimate-task.ts:226-237`); photo chain registered `app.ts:4854-4902`; handler under real PG in `draft-invoice-execution.test.ts` `[suite]` | — |
...
| B10.8 | Audit + undo + idempotency on every mutation | ⚙️ | 4 | 5s window (`lifecycle.ts:53`, `actions.ts:433-438`); DB UNIQUE idempotency (`schema.ts:734,1559`); audit calls throughout handlers (sampled) | Not exhaustively re-audited across all 38 handlers this run |
...
| B7.6 line-item to estimate | ✓ rung 5 | — |
...
| Stripe platform | **partially LIVE** | N/A | N/A | Probe: livemode account "Rivet", 2 enabled webhooks → `serviceosapi-production.up.railway.app/webhooks/stripe` (**duplicate registration** — every event delivered twice; event-ID dedupe absorbs it); webhook-secret set-ness per service UNKNOWN |
...
- ✅ Voice coverage computed and stated (32% strict / 42% functional), with the definitional split disclosed.
