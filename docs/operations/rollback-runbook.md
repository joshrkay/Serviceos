...
- Never run the old `service-os-app` prototype's `supabase_migration.sql` on the canonical database. It was removed with `/experiments` in 2026-07 (see `docs/decisions.md` D-016); only the canonical `packages/api/src/db/schema.ts` migrations apply here.
