...
- [x] **RV-006 (P0)** Image pipeline — DONE, PR #533 (migration 161; sharp processor w/ 80MP limit; buffer-first worker w/ consume-vs-retry taxonomy; enqueue hooks on both attach paths; thumbnailUrl; 24h content_hash dedupe; poll-loop in-flight guard added to app.ts). DEVIATION: HEIC deliberately NOT allowlisted — bundled libheif is AV1-only (HEVC iPhone HEIC undecodable); CaptureSheet produces JPEG via canvas; revisit when codec-capable libvips ships.
...
**Atomic tasks:**
- [x] **RV-020 (P1)** DONE, PR #534 (Codex implementer; AddEntrySheet kept byte-identical to base after review). Generalize capture UI: extract `CameraCapture` from `packages/web/src/components/jobs/AddEntrySheet.tsx` into `packages/web/src/components/attachments/CaptureSheet.tsx` (entity-agnostic: job/invoice/estimate context prop, category chips, ≤3-tap path); wire into job detail; component tests.
- [x] **RV-021 (P1)** DONE, PR #534 (invoice detail section included). Estimate attachments UI: render attachments on `packages/web/src/components/estimates/EstimatesPage.tsx` detail + attach action via CaptureSheet; API client `packages/web/src/api/attachments.ts`; tests.
...
**Atomic tasks:**
- [x] **RV-070 (P2)** DONE, PR #536 (owner-line recognition; ownerSession context flag). Owner-line voice recognition: in telephony inbound, detect caller = owner phone (`tenant_settings.owner_phone`/backup) → FSM enters `owner_session` mode flag in `ai/agents/customer-calling/types.ts` context; tests.
- [x] **RV-071 (P2)** DONE, PR #536 (strict-affirmative confirm + session-level challenge lockout + one-tap refusal fallback). `approve_proposal`/`reject_proposal` voice intents + owner-mode task handler `ai/tasks/proposal-approval-task.ts`: resolve target among pending proposals (extend `ai/resolution/entity-resolver.ts` with pendingProposals source — RV-072), readback (type, customer, amount), require explicit confirmation; calls `approveProposal()`/`rejectProposal()` from `proposals/actions.ts`; money/irreversible add challenge question (last 4 of owner phone? configured PIN in tenant_settings) — config `voice_approval_challenge`; cassettes incl. refusal paths.
- [x] **RV-072 (P2)** DONE, PR #536 (pendingProposals resolver source w/ ordinals). Entity-resolver pendingProposals candidate source (F-3); unit tests for ordinal disambiguation ("the second one").
- [x] **RV-073 (P2)** DONE, PR #536 (all four channels tagged). Audit channel tagging: `approveProposal()` accepts `channel: 'ui'|'sms'|'one_tap'|'voice'` recorded in audit metadata; backfill call sites; tests.
