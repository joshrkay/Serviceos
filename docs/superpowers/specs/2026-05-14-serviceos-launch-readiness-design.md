...
**Current state → gap (MODERATE).** *Built:* Estimate and Invoice models, the shared billing engine, integer-cents line items, Stripe payment-link generation, Stripe webhook signature-verification + parsing, payment recording, and the estimate → invoice conversion handler. *Partial:* the webhook handler parses events but the wiring that auto-updates the invoice and job on `payment succeeded` is not confirmed end-to-end. *Missing:* a denormalized job money-state, overdue detection + event emission, and execution handlers for `reschedule_appointment` / `cancel_appointment` (the proposal types exist but the handlers are stubs). **The gap is the money-state rollup + an overdue cron/event + finishing the webhook → invoice wiring + the reschedule/cancel handlers — the money primitives are all built.**
...
| 6 | Time-to-Cash Chain | Full estimate → invoice chain | MODERATE | Money-state rollup + overdue events + reschedule/cancel handlers |
...
> **Note on gap accuracy:** these gap sizes came from a fast, broad parallel audit. The §4 correction above (the intake page was reported "missing" but is fully built) shows the broad audit can be wrong in either direction. Treat each gap size as a starting estimate — the focused codebase explore done at the start of each plan is the authoritative source, and may revise the spec.
