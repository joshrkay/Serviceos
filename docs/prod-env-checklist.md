...
| `AI_CLASSIFY_INTENT_DEADLINE_MS` | Recommended for voice | Production: `12000`. **Must not be empty string** — present-but-empty silently becomes 4000ms (`check-ai-provider-config` fails). Unset → code default 4000 (dev) |
| `AI_FALLBACK_PROVIDER_API_KEY` + `AI_FALLBACK_PROVIDER_BASE_URL` | Recommended for voice 50/50 | Dual-provider failover (OpenAI primary → OpenRouter). Both required together. Script: `scripts/apply-railway-ai-fallback.sh`. Not a Profile B swap |
| `AI_FALLBACK_LIGHTWEIGHT_MODEL` | Optional with fallback | Default `meta-llama/llama-3.1-8b-instruct` for `classify_intent` on the fallback host |
