...
| Rung 6: which two real tenants — or is 5 the ceiling? — **answered 2026-09-26:** rung 5 is this map's ceiling; rung 6 is the launch effort's first act | #999 (closed) | every row's target rung |
...
### 1.6 own number on own subaccount — rung 5 needs a live Twilio subaccount (from the setup-8-1 lane, issue #1016)
- **What:** what's provable without real credentials IS proven — `test/integrations/twilio-provisioning.test.ts` and 7 sibling unit files (85/85) exercise the subaccount-provisioning write + its audit event against a Twilio stub, and separately (this lane) the same dev-stub path is proven to run for real, automatically, on the in-process queue against real Postgres as part of the 1.8 journey below (`+15005550006` written to `tenant_settings`/phone integration rows, confirmed via psql). That is PROVEN-UNIT / PROVEN-REAL-DB-write-path at best. Rung 5 requires reaching the capability the story actually names — a tenant's OWN number on their OWN Twilio subaccount — which by definition cannot be produced by the stub (`isTwilioTestNumber` / `phone-policy.ts` explicitly forbids ever persisting a `+1500555xxxx` magic number via the real, paid provisioning path).
- **Josh's call:** rung 5 for 1.6 needs a live Twilio account with subaccount-creation credentials (`TWILIO_ACCOUNT_SID`/`TWILIO_AUTH_TOKEN` with subaccount permissions) made available to a lane, OR a decision that 3 (PROVEN-UNIT, capped by the third-party dependency) is this row's ceiling absent that.
- **Until decided:** row 1.6 stays at 3; no lane claims 5 on it.

### 1.10 brand voice Settings surface is gated behind a tenant feature flag with no self-service enable path (from the setup-8-1 lane, issue #1016)
- **What:** 1.10's tone-lock and cool-down are already PROVEN-REAL-DB with T1 (`brand-voice.integration.test.ts`, `update-brand-voice-voice-execution.test.ts`) — rung 4. Reaching it at rung 5 means walking a real browser to the surface the story names: Settings → "Brand voice". That row is rendered ONLY when `me.brand_voice_configurator_enabled` is true (`packages/web/src/components/settings/SettingsPage.tsx:869`), which resolves through `isFlagEnabledForTenant(tenantId, 'brand_voice_configurator')` (`packages/api/src/app.ts:3468`, `:5289`) — a per-tenant override on top of a PLATFORM flag that has **no seed row and no migration inserting it** (`git grep -rn brand_voice_configurator packages/api` finds only the read call sites), so it resolves to OFF for every tenant by default. There is no onboarding step, settings toggle, or other tenant self-service action that turns it on — the only paths are a direct row in the platform `feature_flags`/`tenant_feature_flags` tables (an admin/ops action, or a raw SQL write) or the in-memory dev fallback's `featureFlagRepo.get(flagKey)` returning non-null (which also has no seed). Turning it on for an e2e tenant specifically to reach rung 5 would be exactly the "no SQL setup, no admin route, no env shortcut" rung-5 rule forbids: it would prove reachability past a gate the product does not currently open for anyone.
- **Josh's call:** either (a) an ops-facing way exists or should exist for a tenant/owner to request or self-enable brand-voice-configurator (in which case rung 5 is that path, driven for real), or (b) the flag is intentionally ops-only pre-GA, in which case 1.10 has no rung-5 path until the flag ships default-on or gets a self-service toggle, and 4 is this row's ceiling until then.
- **Until decided:** row 1.10 stays at 4; no lane claims 5 on it. (1.8's rung-5 journey below reaches identity → pack → phone → billing → AI check — the "brand voice" leg named alongside it in #1016 stops here, at this exact gate.)
