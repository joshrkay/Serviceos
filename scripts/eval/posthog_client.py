"""Shared PostHog analytics client for ServiceOS eval scripts.

Initializes a PostHog instance from environment variables and exposes a
module-level `client` (or None when unconfigured) plus `capture` / `capture_exception`.
Uses atexit to flush events on exit automatically.
"""
