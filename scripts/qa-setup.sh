...
#   4. Ensure synthetic HMAC JWT subjects have users rows (idempotent)
#   5. Mint HMAC JWTs
#   6. HMAC auth probe (fail fast, GET /api/me)
#   7. Full qa:doctor gate
#
# Usage:
...
section "Step 1/7 — bootstrap doctor (URLs + DB + HMAC secret)"
npm run qa:doctor:bootstrap

# ── 2. Seed journey fixtures (qa-runner IDs) ───────────────────────────────
section "Step 2/7 — seed journey fixtures (qa-runner)"
...
section "Step 3/7 — seed matrix fixtures (E2E_TENANT_*)"
MATRIX_EXPORTS="$(npx tsx e2e/qa-matrix/fixtures/seed.ts | grep '^export E2E_')"
# shellcheck disable=SC2086
eval "$MATRIX_EXPORTS"
write_qa_local_env "$MATRIX_EXPORTS"

# ── 4. Ensure QA HMAC user rows (idempotent) ──────────────────────────────
# qa:doctor's HMAC probe (and qa-matrix/qa-runbook tokens) need users rows
# for the synthetic clerk_user_ids below. qa:reset wipes them, so this must
# run every time, after tenants exist (step 3) and before the probe (step 5).
section "Step 4/7 — ensure QA HMAC user rows (idempotent)"
npx tsx scripts/ensure-qa-hmac-users.ts

# ── 5. Mint HMAC tokens (journey tenant IDs for qa-runner) ───────────────
section "Step 5/7 — mint HMAC JWTs"
...
# ── 6. HMAC auth probe (fail fast) ───────────────────────────────────────
section "Step 6/7 — HMAC auth probe (GET /api/me)"
...
# ── 7. Full doctor ─────────────────────────────────────────────────────
section "Step 7/7 — full doctor (all 11 vars + HMAC probe)"
npm run qa:doctor
