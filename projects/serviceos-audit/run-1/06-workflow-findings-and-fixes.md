...
### 🔴 Still held / documented — need a migration-strategy or product decision

| Finding | Sev | Verifier's minimal fix + note |
|---|---|---|
| **Vapi webhook cross-tenant forgery** — `/webhooks/vapi/:tenantId` verifies only a single **global** `VAPI_WEBHOOK_SECRET` (same for every tenant) and never binds the signature to `:tenantId`; unlike Twilio/SendGrid which resolve a per-tenant secret + assert `integration.tenantId===tenantId`. Exploit requires knowing the server-side global secret. | high | **Coordinated staged change (deliberately not rushed — a naive switch breaks inbound voice for existing tenants):** (1) add a per-tenant `vapi_webhook_secret` column (tenant config table alongside `vapi_assistant_id`) + migration; (2) generate a random secret per tenant at provisioning (`provision-twilio.ts:326`, pass as `serverUrlSecret`); (3) extend `integrationResolver` to `'vapi'` and, in the route (`routes.ts:2233`), verify against the per-tenant secret **with a temporary fallback to the global secret** so not-yet-re-provisioned tenants keep working; (4) a re-provisioning migration to rotate every existing assistant to its per-tenant secret, then remove the fallback. Steps 1-3 close it for new tenants without breaking voice; step 4 closes it for all. **Owner decision: re-provisioning strategy for live tenants.** |
