...
- [ ] **QA-032** 🔧 P1 — Generate (materialize) due occurrences of a recurring series into real jobs + appointments. The panel has no "generate now" button — this is API-only: `POST /api/recurring-jobs/:id/generate` (optional `{horizonDays}`, default 30, capped 365). **Expect:** one job + appointment per due occurrence in the horizon; re-running the same call doesn't duplicate already-materialized dates (idempotent); an archived series 409s instead of generating.
