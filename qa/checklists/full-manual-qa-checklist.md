...
**Total workflows in this checklist: 109** (13 inbound-call, 4 voice/in-app,
92 manual-click, several overlapping both), spanning 22 feature areas.
...
- [ ] **QA-015** 🖱️ P0 — Create a customer with a service location. **Expect:** appears in `/customers` list and in DB under the correct tenant. (WF-11, `[CUST-01]`)
- [ ] **QA-016** 🖱️ P1 — Edit customer fields + service location; attempt a duplicate. **Expect:** edits persist; dedupe guard triggers or merges as designed. (WF-12 — no dedicated matrix row yet; closest existing coverage is `[CUST-01]` create + `[CUST-03]` archive)
- [ ] **QA-017** 🖱️ P1 — Open a customer's timeline. **Expect:** jobs, estimates, invoices, and comms all show in one place with no 5xx. (WF-13)
- [ ] **QA-018** 🖱️ P2 — Search customers by name, phone, and email. **Expect:** all three match; partial-match search returns results.
- [ ] **QA-019** 🖱️ P1 — Add customer group / tag and filter customer list by it. **Expect:** filter narrows the list correctly.
...
- [ ] **QA-024** 🖱️ P0 — Create a job from a customer record. **Expect:** job appears on `/jobs` and on the customer timeline. (WF-17, `[JOB-01]`)
- [ ] **QA-025** 🖱️ P0 — Walk a job through its full lifecycle: scheduled → in progress → complete. **Expect:** each transition is audit-logged; UI reflects status immediately. (WF-18)
- [ ] **QA-026** 🖱️ P1 — Upload job photos (`/jobs/:id/photos`). **Expect:** photos upload, thumbnail renders, attached to the correct job.
- [ ] **QA-027** 🖱️ P2 — Fill a custom job form and add a custom field. **Expect:** values save and re-render on reopen.
- [ ] **QA-028** 🖱️ P1 — On an approved estimate, use "Convert to job" (`ConvertToJobSheet`). Note estimates are job-first — the estimate already belongs to an existing job, so this schedules/assigns *that* job (auto-picking a technician + slot, or your override) rather than creating a new one. **Expect:** the existing linked job gets scheduled/assigned with no duplicate job created, and the estimate flips to accepted.
- [ ] **QA-029** 🖱️ P1 — Cancel a job mid-lifecycle. **Expect:** status reflects cancellation; any linked appointment is released.

## 6. Schedule & appointments

- [ ] **QA-030** 🖱️ P0 — Create an appointment on the schedule calendar. **Expect:** visible in week view immediately. (WF-19, `[SCH-01]`)
- [ ] **QA-031** 🖱️ P0 — Reschedule an appointment (drag or edit form). **Expect:** window updates, no double-assignment created. (WF-20, `[SCH-02]`)
- [ ] **QA-032** 🖱️ P1 — Cancel an appointment. **Expect:** status canceled, dispatch event fires, technician (if assigned) is freed. (WF-21, `[SCH-03]`)
- [ ] **QA-033** 🖱️ P0 — Attempt to double-book the same technician over an overlapping window. **Expect:** blocked or flagged — the DB exclusion constraint should reject the overlap, not just the UI.
- [ ] **QA-034** 🖱️ P2 — Switch calendar between day/week/month views. **Expect:** appointments render correctly in every view, no layout break.
...
ahead and do QA-093 (Settings → invite a team member as `technician`)
...
appointment first (§6) so both lanes are populated before QA-034 (a drop
target) and QA-037 (reassign between two lanes).

- [ ] **QA-035** 🖱️ P1 — Drag an unassigned job from the pool onto a technician lane on `/dispatch`. **Expect:** assignment proposal created or direct-assigns per config. (WF-23)
- [ ] **QA-036** 🖱️ P1 — Trigger the feasibility preview before confirming an assignment. **Expect:** overlap/travel-time warnings surface when relevant. (WF-24)
- [ ] **QA-037** 🖱️ P1 — Approve a schedule proposal generated from dispatch, from the inbox. **Expect:** proposal executes; calendar updates to match. (WF-25)
- [ ] **QA-038** 🖱️ P1 — Reassign a job from one technician's lane to another. **Expect:** old assignment clears, new one appears, no orphaned assignment left behind.

## 8. Technician / field

- [ ] **QA-039** 🖱️ P1 — As a technician, open `/technician/day` and clock in/out on a job. **Expect:** time entry rows created; weekly hours reflect the entry. (WF-22)
- [ ] **QA-040** 🖱️ P1 — From the mobile tech view (`?view=tech`), change a job's status via the CTA buttons. **Expect:** status updates; optional customer SMS fires if configured. (WF-26, WF-27)
- [ ] **QA-041** 🖱️ P1 — Verify tap targets on `/technician/day` at a 320px-wide viewport. **Expect:** ≥44px targets, no horizontal overflow (per CLAUDE.md mobile UI rule).

## 9. Estimates

- [ ] **QA-042** 🖱️ P0 — Draft an estimate, add line items from the price book. **Expect:** totals in integer cents match manual math; uncatalogued lines get a lower confidence/flag. (WF-28, `[EST-01]`)
- [ ] **QA-043** 🖱️ P0 — Send the estimate to the customer (SMS and/or email). **Expect:** status flips to "sent"; dispatch log row created. (WF-29)
- [ ] **QA-044** 🖱️ P0 — As the customer, open the estimate link (`/e/:id`) and approve it (with a deposit, if configured). **Expect:** status flips to accepted; deposit charge succeeds if required. (WF-30, `[PORT-01]`)
- [ ] **QA-045** 🖱️ P1 — As the customer, open an **expired or invalid** estimate token. **Expect:** a clean error state — no leaked mock/placeholder data.
- [ ] **QA-046** 🖱️ P0 — Convert an accepted estimate to an invoice. **Expect:** `estimate_id` link preserved, line items carry over unchanged, no re-key. (WF-31, `[EST-05]`)
...
- [ ] **QA-048** 🖱️ P0 — As the customer, pay the invoice on `/pay/:id` with a Stripe test card. **Expect:** `checkout.session.completed` webhook flips invoice to paid. (WF-33, `[PORT-02]`)
...
- [ ] **QA-054** 🖱️ P2 — Create a maintenance contract and attach it to a customer. **Expect:** appears on `/contracts` and on the customer's contract detail page.
- [ ] **QA-055** 🖱️ P2 — Edit/cancel a contract. **Expect:** status updates persist; linked jobs (if any) are unaffected.
...
- [ ] **QA-063** 🖱️ P1 — Click a proposal toast from elsewhere in the app. **Expect:** navigates straight into `/inbox` on the right item. (WF-41)
...
- [ ] **QA-078** ☎️ P1 — Let a call ring through to voicemail (or call outside business hours). **Expect:** `voicemail-fallback` picks up cleanly; the message is captured and a transcript shows up on `/interactions`.
- [ ] **QA-079** ☎️ P1 — After the call ends, check `/interactions` for the transcript and recording. **Expect:** transcript is complete and encrypted at rest (per the 2026-06-04 QA report's Blocker 12 fix); recording link works. (WF-44, `[VOX-09]` session-in-timeline, `[VOX-10]` session artifact/DB linkage)
...
- [ ] **QA-085** 🖱️ P2 — Send an email via SendGrid-backed path (estimate/invoice delivery). **Expect:** email arrives, links resolve to the correct tenant-scoped page.
...
`customerId` from your tenant to get a token before QA-088, and again with
a short `ttlDays` (or a hand-edited token) to get an expired/garbage one for
QA-089.

- [ ] **QA-087** 🖱️/🔧 P1 — From inside the customer portal (`/portal/:token`), use the **Request service** tab (`PortalRequestService`) to submit a new request. **Expect:** request recorded and the operator is notified — this is the actual WF-48 flow (do not conflate with QA-086's public `/book`, which is a different, unauthenticated surface). (WF-48)
...
- [ ] **QA-089** 🖱️/🔧 P1 — Open the portal with an expired/garbage token. **Expect:** clean error, no data leak, no 500.
- [ ] **QA-090** 🖱️ P1 — Submit the post-job feedback form (`/feedback/:token`). **Expect:** rating persists; Settings → Feedback dashboard chart updates. (WF-49)
- [ ] **QA-091** 🖱️ P1 — Resubmit the same feedback token twice. **Expect:** second submission is rejected or treated as an edit — not double-counted.

## 19. Settings

- [ ] **QA-092** 🖱️ P1 — Update company profile (name, branding, contact info). **Expect:** changes reflect immediately in customer-facing surfaces (estimate/invoice pages, portal).
- [ ] **QA-093** 🖱️ P1 — Invite a team member and set their role. **Expect:** invite delivers; role gates the right UI/API access after they accept.
- [ ] **QA-094** 🖱️ P0 — Connect/verify Stripe from Settings. **Expect:** Stripe Connect account status shows correctly; payments route to the right connected account.
- [ ] **QA-095** 🖱️ P1 — Edit an SMS/email template. **Expect:** the edited copy is what customers actually receive on the next send.
- [ ] **QA-096** 🖱️ P1 — Add/edit a price-book item and use it in a new estimate. **Expect:** the new price shows up as a selectable catalog line, and AI-drafted lines resolve against it.
- [ ] **QA-097** 🖱️ P2 — Change language setting (`/settings/language`). **Expect:** UI (and voice persona, if wired) reflects the new language.

## 20. Integrations — Google Calendar

- [ ] **QA-098** 🖱️ P1 — Connect Google Calendar from Settings. **Expect:** OAuth completes, integration status shows connected. (WF-50)
- [ ] **QA-099** 🖱️ P1 — Create/assign an appointment and confirm it pushes to Google Calendar. **Expect:** `appointment_calendar_events.status = 'synced'` with a real external event id; event visible in the actual Google Calendar. (proposed matrix row `CAL-01` — not yet added to `matrix.ts`; see the WF-50 appendix in the platform-assessment spec)
- [ ] **QA-100** 🖱️ P2 — Disconnect Google Calendar. **Expect:** integration clears; no further pushes attempted; existing synced events aren't force-deleted from the customer's calendar.
...
- [ ] **QA-101** 🖱️ P0 — Hit `/metrics` and other operational endpoints without an auth token. **Expect:** requires the configured secret, not open by default.
- [ ] **QA-102** 🖱️ P0 — Hammer an authenticated endpoint past the configured rate limit. **Expect:** 429s kick in rather than the request silently succeeding forever.
- [ ] **QA-103** 🖱️ P0 — As Tenant B, attempt to approve/reject a Tenant A proposal by guessing/incrementing its ID. **Expect:** rejected — proposal execution respects tenant scoping, not just list views.
- [ ] **QA-104** 🖱️ P0 — As Tenant B, directly hit Tenant A's customer/job/estimate/invoice detail URLs by ID (repeat QA-005 with fresh IDs from §3–10 of your Tenant A pass). **Expect:** 403/404 on every entity type you touched, not just the ones already covered in §1.
- [ ] **QA-105** 🖱️ P0 — Compare Tenant A's and Tenant B's price books, templates, and settings side by side. **Expect:** each tenant's catalog/templates are fully independent — editing one never mutates the other (regression risk from any shared-cache or shared-fixture bug).
- [ ] **QA-106** ☎️ P0 — Call Tenant A's Twilio number and Tenant B's Twilio number back to back. **Expect:** each call is answered with that tenant's own greeting/persona and only surfaces that tenant's data (e.g. asking "what's my balance" on Tenant B's line never returns a Tenant A customer's numbers) — confirms `identify-caller` and the voice skills are scoped per-tenant, not per-phone-number-format.
- [ ] **QA-107** 🖱️ P1 — Create an estimate/invoice on both tenants around the same time. **Expect:** numbering sequences (invoice #, estimate #) are independent per tenant — no shared counter, no collision.
- [ ] **QA-108** 🖱️ P1 — Compare Tenant A's and Tenant B's Stripe Connect status and a payment made on each. **Expect:** each tenant's payments settle to its own connected Stripe account — a Tenant A customer's payment never appears on Tenant B's money dashboard.

## 22. Misc / internal

- [ ] **QA-109** 🖱️ P2 — Open `/design` (internal design-system showcase). **Expect:** loads without error; not linked from customer-facing nav.
