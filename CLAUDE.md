...
The five canonical roles with default names: `needs-triage`, `needs-info`, `ready-for-agent`, `ready-for-human`, `wontfix`. See `docs/agents/triage-labels.md`.

### Domain docs

Multi-context: a root `CONTEXT-MAP.md` points at one `CONTEXT.md` per package (`packages/api`, `packages/web`, `packages/shared`); system-wide decisions are the D-NNN entries in `docs/decisions.md`. See `docs/agents/domain.md`.
